...
const json = async (relative: string) =>
  Bun.file(path.resolve(root, relative)).json() as Promise<Record<string, unknown>>
...
const required: Array<[string, string[]]> = [
  [
    ...
    [`outfile: \`dist/\${name}/bin/\${Brand.cli}\``, `releaseTag(Script.channel, Script.version)`, `OPENCTRLC_VERSION`],
  ],
  ...
  [
    ...
    ['owner: "ponponon"', 'repo: "openctrlc"', 'artifactName: "openctrlc-'],
  ],
  [
    "packages/opencode/script/publish.ts",
    ["ghcr.io/ponponon/openctrlc", "github.com/ponponon/openctrlc", "ponponon/homebrew-tap"],
  ],
  ["nix/opencode.nix", ["OPENCTRLC_DISABLE_MODELS_FETCH"]],
  [
    "github/action.yml",
    [
      "https://raw.githubusercontent.com/ponponon/openctrlc/dev/install",
      '| OPENCTRLC_INSTALL_DIR="$HOME/.openctrlc/bin" bash',
      "run: openctrlc github run",
    ],
  ],
  ...
  [
    ...
    [
      ...
      'const tag = Script.channel === "beta" ? "beta"',
      "--repo ${repo}",
    ],
  ],
...
try {
  ...
  const runs =
    typeof action === "object" && action !== null && typeof action.runs === "object" && action.runs !== null
      ? (action.runs as { using?: unknown; steps?: unknown })
      : undefined
  ...
  for (const [index, step] of (runs?.steps ?? []).entries()) {
    ...
    if (typeof item.run !== "string" && typeof item.uses !== "string")
      failures.push(`github/action.yml step ${index} must have run or uses`)
    if (typeof item.run === "string" && typeof item.shell !== "string")
      failures.push(`github/action.yml run step ${index} must declare shell`)
  }
} catch (error) {
...
for (const directory of generated) {
  for await (const relative of new Bun.Glob("**/*.ts").scan({ cwd: path.join(root, directory) })) {
    ...
    if (source.includes("@opencode-ai/"))
      failures.push(`${path.join(directory, relative)} contains stale internal imports`)
  }
}
...
for (const directory of audited) {
  const files =
    directory === "install"
      ? [directory]
      : await Array.fromAsync(new Bun.Glob("**/*").scan({ cwd: path.join(root, directory), absolute: true }))
...
for (const file of readmes) {
  ...
  const installation =
    source.match(
      /### (?:Installation|安装|安裝|インストール|설치|Установка|Installasjon|Instalação|Instalación|Installazione|Εγκατάσταση|การติดตั้ง|Інсталяція)[\s\S]*?(?=### |$)/,
    )?.[0] ?? source
  ...
  for (const [index, block] of fenced.entries()) {
    try {
      ...
      if (typeof parsed !== "object" || parsed === null)
        failures.push(`${path.relative(root, file)} fence ${index} is not a YAML object`)
    } catch (error) {

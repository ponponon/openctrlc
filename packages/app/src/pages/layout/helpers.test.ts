...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("openctrlc://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("openctrlc://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("openctrlc://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("openctrlc://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("openctrlc://open-project")).toBeUndefined()
    expect(parseDeepLink("openctrlc://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "openctrlc://open-project?directory=/a",
      "openctrlc://other?directory=/b",
      "openctrlc://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("openctrlc://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("openctrlc://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("openctrlc://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("openctrlc://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "openctrlc://new-session?directory=/a",
      "openctrlc://open-project?directory=/b",
      "openctrlc://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OPENCTRLC__: {
        deepLinks: ["openctrlc://open-project?directory=/a"],
      },
    } as unknown as Window & { __OPENCTRLC__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["openctrlc://open-project?directory=/a"])

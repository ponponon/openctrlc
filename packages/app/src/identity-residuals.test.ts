...
test("all E2E TypeScript fixtures use the expected persistence namespace", async () => {
  ...
  const expectedNoPersistence = new Set([
    ...
    "e2e/performance/timeline-stability/search.spec.ts",
    ...
    "e2e/reproduction/session-search-bar/playwright.config.ts",
    "e2e/reproduction/session-search-bar/session-search-bar.spec.ts",

...
import { wslOpenctrlcAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.openctrlcChecks[item.config.distro]
          const openctrlcAction = () => wslOpenctrlcAction(check())
          const busy = () => wsl.data?.job?.kind === "install-openctrlc" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={openctrlcAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installOpenctrlc(item.config.distro))}
                    >

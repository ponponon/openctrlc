...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-openctrlc" | "add" | null
  loading: boolean
  width: string | null
}
...
export function addServerViewModel(input: {
  ...
  const openctrlcCheck = selectedDistro ? (state?.openctrlcChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.openctrlcChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  openctrlcCheck: WslOpenctrlcCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const probingOpenctrlc =
    probingSelected ||
    (ready &&
      (!input.openctrlcCheck ||
        (!!input.selectedDistro &&
          input.state?.job?.kind === "probe-addable" &&
          input.state.job.distros.includes(input.selectedDistro))))
  const installingOpenctrlc =
    input.state?.job?.kind === "install-openctrlc" && input.state.job.distro === input.selectedDistro
  if (!ready || probingOpenctrlc) {
    return {
      variant: "contrast",
  ...
  if (!addServerOpenctrlcReady(input.openctrlcCheck)) {
    const update = !!input.openctrlcCheck?.resolvedPath && input.openctrlcCheck.matchesDesktop === false
    return {
      variant: "neutral",
...
function addServerOpenctrlcReady(check: WslOpenctrlcCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.openctrlcChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.openctrlcChecks[item.name]) return [`openctrlc:${item.name}`]
    return []
  })

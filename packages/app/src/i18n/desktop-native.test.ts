...
describe("desktop native translations", () => {
  test("uses native language names independent of the active locale", () => {
    expect(DESKTOP_NATIVE_LOCALES).toEqual(["en", "zh", "ja", "ko"])
  ...
  test("rejects unsupported locales and mismatched key sets", () => {
    ...
    expect(parseDesktopNativeBundle({ ...bundle, locale: "de" })).toBeUndefined()
    expect(
      parseDesktopNativeBundle({
        ...bundle,
...
describe("desktop native locale detection", () => {
  test("follows preference order and skips invalid or unsupported tags", () => {
    ...
    expect(detectDesktopNativeLocale(["fr-FR", "ko-KR"])).toBe("ko")
    expect(detectDesktopNativeLocale(["eo", "de-DE"])).toBe("en")
  })

  test("uses the Simplified Chinese bundle for every Chinese locale", () => {
    expect(detectDesktopNativeLocale(["zh-TW"])).toBe("zh")
    expect(detectDesktopNativeLocale(["zh-SG"])).toBe("zh")
    expect(detectDesktopNativeLocale(["ja-JP"])).toBe("ja")
    expect(detectDesktopNativeLocale(["ko-KR"])).toBe("ko")
  })
})

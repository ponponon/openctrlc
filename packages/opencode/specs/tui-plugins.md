...
- For local scope, path is `<worktree>/.openctrlc` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.openctrlc`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.openctrlc`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.openctrlc` dir or `OPENCTRLC_CONFIG_DIR`, OpenCtrlC installs `@opencode-ai/plugin` into that dir and writes:
...
- Local plugins persist installed themes under the local `.openctrlc/themes` area near the plugin config source.
...
- `--pure` / `OPENCTRLC_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.openctrlc/plugins/tui-smoke.tsx`
- Local vim plugin: `.openctrlc/plugins/tui-vim.tsx`
- Local smoke config: `.openctrlc/tui.json`
- Local smoke theme: `.openctrlc/plugins/smoke-theme.json`

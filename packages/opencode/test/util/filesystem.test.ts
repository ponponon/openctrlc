...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "openctrlc.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "openctrlc.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "openctrlc.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "openctrlc.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["openctrlc.json", "openctrlc.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "openctrlc.json"),
        path.join(project, "openctrlc.jsonc"),
      ])
    })
  })

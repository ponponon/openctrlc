...
import { EventSequenceTable } from "@openctrlc/core/event/sql"
...
import { WorkspaceTable } from "@openctrlc/core/control-plane/workspace.sql"
...
const originalEnv = {
  OPENCTRLC_AUTH_CONTENT: process.env.OPENCTRLC_AUTH_CONTENT,
  OPENCTRLC_EXPERIMENTAL_WORKSPACES: process.env.OPENCTRLC_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.OPENCTRLC_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.OPENCTRLC_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.OPENCTRLC_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.OPENCTRLC_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.OPENCTRLC_EXPERIMENTAL_WORKSPACES).toBe("true")

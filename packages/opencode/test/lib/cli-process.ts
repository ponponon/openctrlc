// Subprocess test harness for the OpenCtrlC CLI. Spawns the real binary against
...
//   - OPENCTRLC_CONFIG_CONTENT      : provider config inline, no files to find
//   - OPENCTRLC_TEST_HOME           : pins os.homedir() → tmpdir
//   - OPENCTRLC_DISABLE_PROJECT_CONFIG : skip walking up for openctrlc.json
//   - OPENCTRLC_PURE                : skip external plugin discovery + install
//   - OPENCTRLC_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    OPENCTRLC_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    OPENCTRLC_CONFIG_CONTENT: configJson,
    OPENCTRLC_DISABLE_PROJECT_CONFIG: "1",
    OPENCTRLC_PURE: "1",
    OPENCTRLC_DISABLE_AUTOUPDATE: "1",
    OPENCTRLC_DISABLE_AUTOCOMPACT: "1",
    OPENCTRLC_DISABLE_MODELS_FETCH: "1",
    OPENCTRLC_AUTH_CONTENT: "{}",
  }
}
  ...
  return Effect.gen(function* () {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          OPENCTRLC_CONFIG_CONTENT: JSON.stringify({

...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const cacheFile = path.join(tmp, ".git", Brand.runtimeDirectory)
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", Brand.runtimeDirectory)
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, Brand.runtimeDirectory)
      const wrongCache = path.join(parentDir, ".git", Brand.runtimeDirectory)
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, Brand.runtimeDirectory)
      const cacheB = path.join(bareB, Brand.runtimeDirectory)
      const wrongCache = path.join(parentDir, ".git", Brand.runtimeDirectory)

...
import { createOpencodeClient } from "@openctrlc/sdk/v2"
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.OPENCTRLC_SERVER_PASSWORD = input?.password
      Flag.OPENCTRLC_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".openctrlc", "skills", "project-rest-skill", "SKILL.md"),
      `---

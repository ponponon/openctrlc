...
const original = {
  ...
  envPassword: process.env.OPENCTRLC_SERVER_PASSWORD,
  envUsername: process.env.OPENCTRLC_SERVER_USERNAME,
}
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.OPENCTRLC_SERVER_PASSWORD
  else process.env.OPENCTRLC_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.OPENCTRLC_SERVER_USERNAME
  else process.env.OPENCTRLC_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.OPENCTRLC_SERVER_PASSWORD = auth.password
  Flag.OPENCTRLC_SERVER_USERNAME = auth.username
  process.env.OPENCTRLC_SERVER_PASSWORD = auth.password
  process.env.OPENCTRLC_SERVER_USERNAME = auth.username
...
async function startNoAuthListener() {
  ...
  Flag.OPENCTRLC_SERVER_USERNAME = auth.username
  delete process.env.OPENCTRLC_SERVER_PASSWORD
  process.env.OPENCTRLC_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
describe("HttpApi Server.listen", () => {
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.OPENCTRLC_DISABLE_DEFAULT_PLUGINS
    process.env.OPENCTRLC_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.OPENCTRLC_DISABLE_DEFAULT_PLUGINS
      else process.env.OPENCTRLC_DISABLE_DEFAULT_PLUGINS = previous
    }
  })

  ...
  Effect.gen(function* () {
    ...
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        ...
        restoreEnv("OPENCTRLC_SERVER_PASSWORD", original.envPassword)
        restoreEnv("OPENCTRLC_SERVER_USERNAME", original.envUsername)
      }),
    )
  }),
)
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            OPENCTRLC_SERVER_PASSWORD: input?.password,
            OPENCTRLC_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),

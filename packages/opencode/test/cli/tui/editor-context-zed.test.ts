...
import { isZedTerminal, offsetToPosition, resolveZedDbPath, resolveZedSelection } from "@openctrlc/tui/editor-zed"
...
test("resolveZedDbPath skips candidates that cannot be stated", async () => {
  ...
  const previous = process.env.OPENCTRLC_ZED_DB
  process.env.OPENCTRLC_ZED_DB = loop

  try {
    expect(resolveZedDbPath()).toBeUndefined()
  } finally {
    if (previous === undefined) delete process.env.OPENCTRLC_ZED_DB
    else process.env.OPENCTRLC_ZED_DB = previous
    home.mockRestore()
  }
})

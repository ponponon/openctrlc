...
it.instance("plan agent denies edits except .openctrlc/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".openctrlc/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
`,
      ...
      const home = process.env.OPENCTRLC_TEST_HOME
      process.env.OPENCTRLC_TEST_HOME = test.directory

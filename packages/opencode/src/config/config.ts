...
import type { ConsoleState } from "@openctrlc/core/v1/config/console-state"
import { FSUtil } from "@openctrlc/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@openctrlc/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@openctrlc/core/v1/config/config"
import { RemoteAuthError } from "@openctrlc/core/v1/config/error"
import { ConfigPermissionV1 } from "@openctrlc/core/v1/config/permission"
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.OPENCTRLC_CONFIG && !Flag.OPENCTRLC_CONFIG_DIR && !Flag.OPENCTRLC_CONFIG_CONTENT) {
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "OPENCTRLC_CONFIG_CONTENT") return "local"
        ...
        if (Flag.OPENCTRLC_CONFIG) {
          yield* merge(Flag.OPENCTRLC_CONFIG, yield* loadFile(Flag.OPENCTRLC_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.OPENCTRLC_CONFIG })
        }
        ...
        if (Flag.OPENCTRLC_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from OPENCTRLC_CONFIG_DIR", { path: Flag.OPENCTRLC_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(Brand.projectDirectory) || dir === Flag.OPENCTRLC_CONFIG_DIR) {
          ...
          // Auto-discovered plugins under the project directory are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.OPENCTRLC_CONFIG_CONTENT) {
          const source = "OPENCTRLC_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.OPENCTRLC_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from OPENCTRLC_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["OPENCTRLC_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("OPENCTRLC_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.OPENCTRLC_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.OPENCTRLC_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("OPENCTRLC_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.OPENCTRLC_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.OPENCTRLC_DISABLE_PRUNE) {

...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, project directories
  // between cwd and home, and OPENCTRLC_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.OPENCTRLC_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit OPENCTRLC_TUI_CONFIG override, if set.
  if (Flag.OPENCTRLC_TUI_CONFIG) {
    const configFile = Flag.OPENCTRLC_TUI_CONFIG
  ...
  // 4. Project directories (and OPENCTRLC_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(Brand.projectDirectory) || dir === Flag.OPENCTRLC_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(Brand.projectDirectory) && dir !== Flag.OPENCTRLC_CONFIG_DIR) continue

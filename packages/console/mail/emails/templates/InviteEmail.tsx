...
const CONSOLE_URL = "https://openctrlc.pages.dev/"
...
export const InviteEmail = ({
  ...
  return (
    <Html lang="en">
      <Head>
        <Title>{`OpenCtrlC — ${messagePlain}`}</Title>
      </Head>
        ...
        <Container style={container}>
          <Section style={frame}>
            <Row>
              <Column>
                <A href={CONSOLE_URL}>
                  <Img height="32" alt="OpenCtrlC Logo" src={`${assetsUrl}/logo.png`} />
                </A>
              </Column>
            </Row>

            <Section style={{ padding: `${unit * 2}px 0 0 0` }}>
              <Text style={headingText}>Join your team's OpenCtrlC workspace</Text>
              <Text style={contentText}>
                You have been invited by <Span style={contentHighlightText}>{inviter}</Span> to join the{" "}
                <Span style={contentHighlightText}>{workspaceName}</Span> workspace on OpenCtrlC.
              </Text>
            </Section>

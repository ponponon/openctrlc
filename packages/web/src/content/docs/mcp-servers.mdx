...
You can add external tools to OpenCtrlC using the _Model Context Protocol_, or MCP. OpenCtrlC supports both local and remote servers.
...
You can define MCP servers in your [OpenCtrlC Config](https://openctrlc.pages.dev/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
OpenCtrlC automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, OpenCtrlC will:
...
If the server requires authentication, OpenCtrlC will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `openctrlc mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, OpenCtrlC will store the tokens securely in `~/.local/share/openctrlc/mcp-auth.json`.
...
Your MCPs are available as tools in OpenCtrlC, alongside built-in tools. So you can manage them through the OpenCtrlC config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect OpenCtrlC to your Sentry account.

...
There are two types of agents in OpenCtrlC; primary agents and subagents.
...
OpenCtrlC comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
OpenCtrlC comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

OpenCtrlC comes with two built-in primary agents and three built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into OpenCtrlC's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
Configure agents in your `openctrlc.json` config file:
...
If no temperature is specified, OpenCtrlC uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global OpenCtrlC config and the project specific config.
...
The model ID in your OpenCtrlC config uses the format `provider/model-id`. Use the provider and model IDs shown by the `/models` picker.
...
:::tip
Run `openctrlc models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/ponponon/openctrlc/pulls).
:::

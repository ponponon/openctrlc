...
Agent skills omogucavaju OpenCtrlC da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
OpenCtrlC pretrazuje ove lokacije:

- Konfiguracija projekta: `.openctrlc/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/openctrlc/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, OpenCtrlC ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.openctrlc/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/openctrlc/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.openctrlc/skills/git-release/SKILL.md` ovako:
...
OpenCtrlC navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `openctrlc.json`:
...
**Za ugradene agente** (u `openctrlc.json`):

---
title: Konfiguracija
description: Korištenje OpenCtrlC JSON konfiguracije.
---

Možete konfigurirati OpenCtrlC koristeći JSON konfiguracijski fajl.

---

## Format

OpenCtrlC podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
2. **Globalna konfiguracija** (`~/.config/openctrlc/openctrlc.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`OPENCTRLC_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`openctrlc.json` u projektu) - postavke specifične za projekat
5. **`.openctrlc` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`OPENCTRLC_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.openctrlc` i `~/.config/openctrlc` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Postavite svoju globalnu OpenCtrlC konfiguraciju u `~/.config/openctrlc/openctrlc.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/openctrlc/tui.json`.
...
Dodajte `openctrlc.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se OpenCtrlC pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `OPENCTRLC_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `OPENCTRLC_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.openctrlc` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.openctrlc`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`openctrlc.pages.dev/config.json`**](https://openctrlc.pages.dev/config.json).

TUI konfiguracija koristi [**`openctrlc.pages.dev/tui.json`**](https://openctrlc.pages.dev/tui.json).
...
Koristite `OPENCTRLC_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `openctrlc.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `openctrlc serve` i `openctrlc web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš OpenCtrlC server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `openctrlc.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj OpenCtrlC konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, OpenCtrlC pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/openctrlc/agents/` ili `.openctrlc/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, OpenCtrlC će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`openctrlc run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/openctrlc/commands/` ili `.openctrlc/commands/`. [Saznajte više ovdje](/docs/commands).
...
OpenCtrlC će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, OpenCtrlC **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju OpenCtrlC sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.openctrlc/plugins/` ili `~/.config/openctrlc/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite OpenCtrlC da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.

---
title: Dodaci
description: Napišite vlastite dodatke za proširenje OpenCtrlC.
---

Dodaci vam omogućavaju da proširite OpenCtrlC spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja OpenCtrlC.
...
- `.openctrlc/plugins/` - Dodaci na nivou projekta
- `~/.config/openctrlc/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/openctrlc/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/openctrlc/openctrlc.json`)
2. Konfiguracija projekta (`openctrlc.json`)
3. Globalni direktorij dodataka (`~/.config/openctrlc/plugins/`)
4. Direktorij dodataka projekta (`.openctrlc/plugins/`)
...
OpenCtrlC pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje OpenCtrlC.
...
Spriječite openctrlc da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u openctrlc:
...
Pomoćnik `tool` kreira prilagođeni alat koji openctrlc može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za openctrlc zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://openctrlc.pages.dev/docs/sdk) za detalje.

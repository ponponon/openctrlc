...
Možete dodati vanjske alate u OpenCtrlC koristeći _Model Context Protocol_, ili MCP. OpenCtrlC podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [OpenCtrlC Config](https://openctrlc.pages.dev/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
OpenCtrlC automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, OpenCtrlC će:
...
Ako server zahtijeva autentifikaciju, OpenCtrlC će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `openctrlc mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, OpenCtrlC sigurno čuva tokene u `~/.local/share/openctrlc/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u OpenCtrlC, zajedno s ugrađenim alatima. Možete njima upravljati kroz OpenCtrlC konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete OpenCtrlC sa vašim Sentry nalogom.

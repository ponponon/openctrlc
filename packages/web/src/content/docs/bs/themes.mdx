...
U OpenCtrlC mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, OpenCtrlC koristi nasu `openctrlc` temu.
...
OpenCtrlC dolazi sa vise ugradenih tema.
...
- Zele da OpenCtrlC odgovara izgledu njihovog terminala
...
OpenCtrlC podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/openctrlc/themes/*.json` ili `$XDG_CONFIG_HOME/openctrlc/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.openctrlc/themes/*.json`
4. **Trenutni radni direktorij** - `./.openctrlc/themes/*.json`

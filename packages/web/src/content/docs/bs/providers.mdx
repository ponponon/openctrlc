---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u OpenCtrlC.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenCtrlC koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj OpenCtrlC konfiguraciji.
...
u `~/.local/share/openctrlc/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem OpenCtrlC
config.
...
OpenCtrlC Zen je lista modela koje je obezbedio OpenCtrlC tim koji su bili
testirano i potvrđeno da dobro radi sa OpenCtrlC. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa OpenCtrlC Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite openctrlc i idite na [openctrlc.ai/auth](https://opencode.ai/auth).
...
Radi kao i svaki drugi provajder u OpenCtrlC i potpuno je opcionalan za korištenje.

---

## OpenCtrlC Go

OpenCtrlC Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža OpenCtrlC tim i koji su testirani i verificirani da dobro rade s OpenCtrlC-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `OpenCtrlC Go` i idite na [openctrlc.ai/auth](https://opencode.ai/zen).
...
Da biste koristili Amazon Bedrock s OpenCtrlC:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete openctrlc:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `openctrlc.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u OpenCtrlC.
:::
...
Možete konfigurirati openctrlc za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi openctrlc ispravno radio.
   :::
...
Također možete dodati modele kroz svoju openctrlc konfiguraciju.
...
da pohrani token u openctrlc auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
OpenCtrlC koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate OpenCtrlC
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`openctrlc.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `openctrlc.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s openctrlc:
...
Za korištenje Google Vertex AI s OpenCtrlC:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće openctrlc.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem openctrlca, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki OpenCtrlC razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki OpenCtrlC razgovor naveden kao zasebna sesija.
...
Možete konfigurirati openctrlc za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati openctrlc za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati openctrlc za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za OpenCtrlC. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s OpenCtrlC:
...
3. Kopirajte API ključ za korištenje u OpenCtrlC.
...
6. **Važno**: Prije upotrebe modela oblaka u OpenCtrlC, morate lokalno povući informacije o modelu:
...
OpenCtrlC Zen je lista testiranih i verifikovanih modela koju je obezbedio OpenCtrlC tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>OpenCtrlC Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **OpenCtrlC Zen**.

```txt
   /connect
```

3. Unesite svoj OpenCtrlC API ključ.
...
Također možete dodati dodatne modele putem vaše openctrlc konfiguracije.
...
5. Također ih možete prilagoditi putem vaše openctrlc konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju openctrlc konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s openctrlc-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `openctrlc.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju OpenCtrlC da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `openctrlc auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite OpenCtrlC konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj openctrlc konfiguraciji.

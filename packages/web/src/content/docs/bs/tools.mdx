...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. OpenCtrlC dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u OpenCtrlC.
...
:::note
Ovaj alat je dostupan samo kada je `OPENCTRLC_EXPERIMENTAL_LSP_TOOL=true` (ili `OPENCTRLC_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz OpenCtrlC provajdera ili kada je varijabla `OPENCTRLC_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju OpenCtrlC:

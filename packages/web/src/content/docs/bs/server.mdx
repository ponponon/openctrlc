---
title: Server
description: Komunicirajte s openctrlc serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `openctrlc serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji openctrlc klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `openctrlc.local` |
...
Postavite `OPENCTRLC_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `openctrlc`, ili postavite `OPENCTRLC_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `openctrlc serve` i za `openctrlc web`.
...
Kada pokrenete `openctrlc`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite openctrlc server za programsku interakciju sa openctrlc-om.
:::

Ova arhitektura omogucava openctrlc podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `openctrlc serve` da startate standalone server. Ako je openctrlc TUI vec pokrenut, `openctrlc serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste OpenCtrlC [IDE](/docs/ide) pluginovi.
...
openctrlc server izlaže sljedece API-je.

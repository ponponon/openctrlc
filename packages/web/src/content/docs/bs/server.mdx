...
Komanda `openctrlc serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji opencode klijent moze koristiti.
...
:::tip
Koristite openctrlc server za programsku interakciju sa opencode-om.
:::

Ova arhitektura omogucava opencode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `openctrlc serve` da startate standalone server. Ako je opencode TUI vec pokrenut, `openctrlc serve` ce pokrenuti novi server.

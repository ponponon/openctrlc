...
OpenCode pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `openctrlc auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/openctrlc/auth.json`.
...
Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti OpenCode-a bez TUI interfejsa. Postavite `OPENCTRLC_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `openctrlc`).
...
Ovo pokreće HTTP server i otvara web pretraživač za pristup OpenCode-u preko web interfejsa. Postavite `OPENCTRLC_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `openctrlc`).

...
Da biste otklonili probleme s OpenCtrlC, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\openctrlc\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `openctrlc --log-level DEBUG`.

---

## Pohrana

OpenCtrlC pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/openctrlc/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\openctrlc`
...
OpenCtrlC Desktop pokreće lokalni OpenCtrlC server (`openctrlc-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `OpenCtrlC` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/openctrlc/openctrlc.jsonc` (ili `~/.config/openctrlc/openctrlc.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/openctrlc/openctrlc.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\openctrlc\openctrlc.jsonc`
...
OpenCtrlC također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/openctrlc/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\openctrlc\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga OpenCtrlC mogao ponovo izgraditi.

1. Potpuno zatvorite OpenCtrlC Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/openctrlc`
- **Linux**: obrišite `~/.cache/openctrlc` (ili pokrenite `rm -rf ~/.cache/openctrlc`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\openctrlc`

3. Ponovo pokrenite OpenCtrlC Desktop.

---

### Rješavanje problema sa vezom na serveru

OpenCtrlC Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `openctrlc.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `OPENCTRLC_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `OPENCTRLC_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, OpenCtrlC Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa OpenCtrlC karakteristikama.

---

### Obavještenja se ne prikazuju

OpenCtrlC Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za OpenCtrlC u postavkama vašeg OS-a, i
...
1. Zatvorite OpenCtrlC Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka OpenCtrlC Desktop aplikacije):

- `openctrlc.settings.dat` (URL zadanog servera za desktop)
- `openctrlc.global.dat` i `openctrlc.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s OpenCtrlC:
...
Da saznate kojim modelima imate pristup, pokrenite `openctrlc models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\openctrlc`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. OpenCtrlC dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\openctrlc`

2. Ponovo pokrenite OpenCtrlC da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti OpenCtrlC da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
OpenCtrlC će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.

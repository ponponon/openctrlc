...
OpenCtrlC Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u OpenCtrlC-u. Pretplatite se na OpenCtrlC Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili OpenCtrlC.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa OpenCtrlC-om.
:::
...
OpenCtrlC Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

OpenCtrlC Go radi kao bilo koji drugi provajder u OpenCtrlC-u.

1. Prijavite se na **<a href={console}>OpenCtrlC Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `OpenCtrlC Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na OpenCtrlC Go.
:::
...
OpenCtrlC Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj OpenCtrlC konfiguraciji
...
Napravili smo OpenCtrlC Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz OpenCtrlC koristite i bilo kojeg drugog provajdera.

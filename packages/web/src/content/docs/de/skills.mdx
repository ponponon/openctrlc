...
Agent Skills erlauben OpenCtrlC, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
OpenCtrlC sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft OpenCtrlC vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.openctrlc/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/openctrlc/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.openctrlc/skills/git-release/SKILL.md` zum Beispiel so:
...
OpenCtrlC listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `openctrlc.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `openctrlc.json`):

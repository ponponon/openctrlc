...
Es gibt zwei Arten von Agenten in OpenCtrlC; Primäragenten und Subagenten.
...
OpenCtrlC verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `openctrlc.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet OpenCtrlC modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale OpenCtrlC-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer OpenCtrlC-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [Ihren konfigurierten Anbieter](/docs/providers) verwenden, würden Sie `opencode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `openctrlc models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/ponponon/openctrlc).
:::

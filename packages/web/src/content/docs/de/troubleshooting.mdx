...
Wenn OpenCtrlC Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\openctrlc\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `openctrlc --log-level DEBUG`.

---

## Speicher

openctrlc speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/openctrlc/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\openctrlc`
...
OpenCtrlC Desktop startet im Hintergrund einen lokalen OpenCtrlC-Server (`openctrlc-cli`-Sidecar).
...
- Nur macOS: `OpenCtrlC`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/openctrlc/openctrlc.jsonc` (oder `~/.config/openctrlc/openctrlc.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/openctrlc/openctrlc.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\openctrlc\openctrlc.jsonc`
...
OpenCtrlC kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/openctrlc/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\openctrlc\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit OpenCtrlC ihn neu aufbauen kann.

1. Beende OpenCtrlC Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/openctrlc`
- **Linux**: loesche `~/.cache/openctrlc` (oder fuehre aus: `rm -rf ~/.cache/openctrlc`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\openctrlc`

3. Starte OpenCtrlC Desktop neu.

---

### Server-Verbindungsprobleme beheben

OpenCtrlC Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `openctrlc.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `OPENCTRLC_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `OPENCTRLC_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt OpenCtrlC Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit OpenCtrlC-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

OpenCtrlC Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer OpenCtrlC in den OS-Einstellungen aktiviert sind, und
...
1. Beende OpenCtrlC Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von OpenCtrlC Desktop):

- `openctrlc.settings.dat` (desktop default server URL)
- `openctrlc.global.dat` und `openctrlc.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit OpenCtrlC hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `openctrlc models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\openctrlc`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. openctrlc installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\openctrlc`

2. Starte openctrlc neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt openctrlc, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
openctrlc erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).

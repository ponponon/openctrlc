---
title: Regeln
description: Lege eigene Anweisungen fuer openctrlc fest.
---

Du kannst openctrlc ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in openctrlc den Befehl `/init` aus.
...
So kann openctrlc sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht openctrlc sie zu erweitern.
...
openctrlc kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/openctrlc/AGENTS.md` speichern.
Diese gelten in allen openctrlc-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt OpenCtrlC dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/openctrlc/AGENTS.md` existiert)
...
Beim Start sucht openctrlc in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/openctrlc/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `openctrlc.json` oder in `~/.config/openctrlc/openctrlc.json` angeben.
...
openctrlc parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit openctrlc.json

Empfohlen ist das `instructions`-Feld in der `openctrlc.json`:
...
Du kannst openctrlc explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `openctrlc.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::

...
Sie können externe Tools zu OpenCtrlC hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. OpenCtrlC unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [OpenCtrlC Config](https://openctrlc.pages.dev/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
OpenCtrlC übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird OpenCtrlC Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von OpenCtrlC beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `openctrlc mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert OpenCtrlC die Token sicher in `~/.local/share/openctrlc/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in OpenCtrlC verfügbar. Sie können sie also wie jedes andere Tool über die OpenCtrlC-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und OpenCtrlC mit Ihrem Sentry-Konto zu verbinden.

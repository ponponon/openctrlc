---
title: Server
description: Nutze den openctrlc-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `openctrlc serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein openctrlc-Client nutzen kann.
...
Setze `OPENCTRLC_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `openctrlc`, kann aber mit `OPENCTRLC_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `openctrlc serve` und `openctrlc web`.
...
Wenn du `openctrlc` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den openctrlc-Server, um openctrlc programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `openctrlc serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `openctrlc serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die OpenCtrlC-[IDE](/docs/ide)-Plugins.
...
Der openctrlc-Server stellt folgende APIs bereit.

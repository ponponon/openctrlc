---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um OpenCtrlC zu erweitern.
---

Mit Plugins können Sie OpenCtrlC erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von OpenCtrlC zu ändern.
...
- `.openctrlc/plugins/` – Plugins auf Projektebene
- `~/.config/openctrlc/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/openctrlc/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/openctrlc/openctrlc.json`)
2. Projektkonfiguration (`openctrlc.json`)
3. Globales Plugin-Verzeichnis (`~/.config/openctrlc/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.openctrlc/plugins/`)
...
OpenCtrlC führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein OpenCtrlC-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie OpenCtrlC erweitern können.
...
:::note
Wenn Sie die OpenCtrlC-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass OpenCtrlC `.env`-Dateien liest:
...
Plugins können OpenCtrlC auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das OpenCtrlC aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für OpenCtrlC zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://openctrlc.pages.dev/docs/sdk).

...
Mit OpenCtrlC kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet OpenCtrlC das `openctrlc`-Theme.
...
OpenCtrlC liefert mehrere eingebaute Themes mit.
...
- OpenCtrlC optisch ans Terminal anpassen wollen
...
OpenCtrlC unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/openctrlc/themes/*.json` oder `$XDG_CONFIG_HOME/openctrlc/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.openctrlc/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.openctrlc/themes/*.json`

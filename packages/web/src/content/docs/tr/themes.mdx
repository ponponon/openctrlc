...
openctrlc ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak openctrlc kendi `openctrlc` temasını kullanır.
...
openctrlc birden fazla yerleşik temayla gelir.
...
- openctrlc'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
openctrlc, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/openctrlc/themes/*.json` veya `$XDG_CONFIG_HOME/openctrlc/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.openctrlc/themes/*.json`
4. **Geçerli çalışma dizini** - `./.openctrlc/themes/*.json`

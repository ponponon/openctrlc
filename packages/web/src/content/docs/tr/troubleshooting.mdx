...
opencode Desktop arka planda yerel bir opencode sunucusu (`openctrlc-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\opencode\openctrlc.jsonc` yapistirin

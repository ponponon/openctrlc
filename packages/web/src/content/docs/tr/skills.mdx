...
Ajan becerileri, openctrlc'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
openctrlc su konumlari tarar:

- Proje konfigurasyonu: `.openctrlc/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/openctrlc/skills/<name>/SKILL.md`
...
Proje yerel yollarinda openctrlc, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.openctrlc/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/openctrlc/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.openctrlc/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
openctrlc, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`openctrlc.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`openctrlc.json` icinde):

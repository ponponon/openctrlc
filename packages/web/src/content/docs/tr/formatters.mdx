...
description: openctrlc dile özgü biçimlendiriciler kullanır.
---

openctrlc, dosyaları dile özgü formatlayıcılar kullanılarak yazıldıktan veya düzenlendikten sonra otomatik olarak formatlar. Bu, oluşturulan kodun projenizin kod stillerine uymasını sağlar.

---

## Yerleşik

openctrlc, popüler diller ve çerçeveler için çeşitli yerleşik biçimlendiricilerle birlikte gelir. Aşağıda ihtiyaç duyduğu biçimlendiricilerin, desteklenen dosya uzantılarının ve komutların veya yapılandırma seçeneklerinin bir listesi bulunmaktadır.
...
Yani eğer projenizin `package.json` dosyasında `prettier` varsa, openctrlc bunu otomatik olarak kullanacaktır.

---

## Nasıl çalışır?

openctrlc bir dosyayı yazdığında veya düzenlediğinde:
...
Biçimlendiricileri openctrlc yapılandırmanızdaki `formatter` bölümü aracılığıyla özelleştirebilirsiniz.

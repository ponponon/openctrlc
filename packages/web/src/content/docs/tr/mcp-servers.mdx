...
_Model Bağlam Protokolü_ veya MCP kullanarak openctrlc'a harici araçlar ekleyebilirsiniz. openctrlc hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [OpenCtrlC Config](https://openctrlc.pages.dev/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
openctrlc, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde openctrlc şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa openctrlc, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `openctrlc mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından openctrlc, belirteçleri `~/.local/share/openctrlc/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra openctrlc'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi openctrlc yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve openctrlc'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.

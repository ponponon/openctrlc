...
`openctrlc serve` komutu, opencode istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
`openctrlc serve` ile bağımsız bir sunucu başlatabilirsiniz. opencode TUI açıksa, `openctrlc serve` yeni bir sunucu başlatır.

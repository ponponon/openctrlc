...
OpenCtrlC Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, OpenCtrlC'daki diğer sağlayıcılar gibi çalışır. OpenCtrlC Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve OpenCtrlC'u kullanmak için bunu kullanmanıza gerek yoktur.
...
OpenCtrlC Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

OpenCtrlC Go, OpenCtrlC'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>OpenCtrlC Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `OpenCtrlC Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye OpenCtrlC Go'ya abone olabilir.
:::
...
OpenCtrlC Go aşağıdaki limitleri içerir:
...
OpenCtrlC yapılandırmanızdaki [model id](/docs/config/#models) formatı `opencode-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `opencode-go/kimi-k3` kullanmalısınız.
...
OpenCtrlC Go'yu şu amaçlarla oluşturduk:
...
4. OpenCtrlC ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.

...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. openctrlc yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
openctrlc içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `OPENCTRLC_EXPERIMENTAL_LSP_TOOL=true` (veya `OPENCTRLC_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca openctrlc provider kullanılırken veya `OPENCTRLC_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

openctrlc başlatırken etkinleştirmek için:

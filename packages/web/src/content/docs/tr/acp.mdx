---
title: ACP Desteği
description: openctrlc'u ACP uyumlu herhangi bir editörde kullanın.
---

openctrlc, [Agent Client Protocol](https://agentclientprotocol.com) veya (ACP) dosyalarını destekleyerek onu doğrudan uyumlu editörlerde ve IDE'lerde kullanmanızı sağlar.
...
openctrlc'u ACP aracılığıyla kullanmak için düzenleyicinizi `openctrlc acp` komutunu çalıştıracak şekilde yapılandırın.

Komut, openctrlc'u, editörünüzle stdio aracılığıyla JSON-RPC üzerinden iletişim kuran ACP uyumlu bir alt süreç olarak başlatır.
...
Açmak için AI Chat agent seçicisindeki yeni 'OpenCtrlC' ajanını kullanın.
...
openctrlc'u [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim)'de ACP ajanı olarak kullanmak için Neovim yapılandırmanıza aşağıdakileri ekleyin:
...
Bu yapılandırma, CodeCompanion'ı openctrlc'u sohbet için ACP ajanı olarak kullanacak şekilde ayarlar.
...
openctrlc, terminalde olduğu gibi ACP aracılığıyla aynı şekilde çalışır. Tüm özellikler desteklenir:
...
- openctrlc yapılandırmanızda yapılandırılmış MCP sunucuları

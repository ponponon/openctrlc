---
title: Eklentiler
description: OpenCtrlC'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek OpenCtrlC'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya OpenCtrlC'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.openctrlc/plugins/` - Proje düzeyinde eklentiler
- `~/.config/openctrlc/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/openctrlc/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/openctrlc/openctrlc.json`)
2. Project config (`openctrlc.json`)
3. Global eklenti dizini (`~/.config/openctrlc/plugins/`)
4. Proje eklenti dizini (`.openctrlc/plugins/`)
...
OpenCtrlC bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir OpenCtrlC SDK istemcisi.
...
OpenCtrlC'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
OpenCtrlC masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
OpenCtrlC'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca OpenCtrlC'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, OpenCtrlC'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra OpenCtrlC'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://openctrlc.pages.dev/docs/sdk) bakın.

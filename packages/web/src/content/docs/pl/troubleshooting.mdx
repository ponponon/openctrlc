...
opencode Desktop uruchamia w tle lokalny serwer opencode (sidecar `openctrlc-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\opencode\openctrlc.jsonc`

...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. OpenCtrlC posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w OpenCtrlC.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `OPENCTRLC_EXPERIMENTAL_LSP_TOOL=true` (lub `OPENCTRLC_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy OpenCtrlC lub gdy zmienna środowiskowa `OPENCTRLC_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu OpenCtrlC:

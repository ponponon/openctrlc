...
Umiejętności agenta pozwalają openctrlc odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
openctrlc przeszukuje te lokalizacje:

- Project config: `.openctrlc/skills/<name>/SKILL.md`
- Global config: `~/.config/openctrlc/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu openctrlc przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.openctrlc/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/openctrlc/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.openctrlc/skills/git-release/SKILL.md` w ten sposób:
...
openctrlc wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `openctrlc.json`:
...
**Dla agentów wbudowanych** (w `openctrlc.json`):

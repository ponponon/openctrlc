---
title: Konfiguracja
description: Ustawienia OpenCtrlC w pliku JSON.
---

Możesz dostosować OpenCtrlC za pomocą pliku konfiguracyjnego JSON.

---

## Format

OpenCtrlC obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "openctrlc"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
2. **Konfiguracja globalna** (`~/.config/openctrlc/openctrlc.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `OPENCTRLC_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`openctrlc.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.openctrlc`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `OPENCTRLC_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.openctrlc` i `~/.config/openctrlc` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Umieść swoją globalną konfigurację OpenCtrlC w `~/.config/openctrlc/openctrlc.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/openctrlc/tui.json`.
...
Dodaj `openctrlc.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy OpenCtrlC się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `OPENCTRLC_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `OPENCTRLC_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.openctrlc` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.openctrlc`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`openctrlc.pages.dev/config.json`**](https://openctrlc.pages.dev/config.json).
...
Użyj `OPENCTRLC_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `openctrlc.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `openctrlc serve` i `openctrlc web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer OpenCtrlC.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `openctrlc.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie OpenCtrlC wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/openctrlc/agents/` lub `.openctrlc/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, OpenCtrlC powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`openctrlc run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/openctrlc/commands/` lub `.openctrlc/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
OpenCtrlC może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy OpenCtrlC nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie OpenCtrlC **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają OpenCtrlC o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.openctrlc/plugins/` lub `~/.config/openctrlc/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie OpenCtrlC tylko do określonych dostawców.

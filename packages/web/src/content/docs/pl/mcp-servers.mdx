...
Możesz dodać dodatkowe informacje do openctrlc za pomocą _Model Context Protocol_, w skrócie MCP. openctrlc obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [OpenCtrlC Config](https://openctrlc.pages.dev/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
openctrlc automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, openctrlc:
...
Jeśli serwer wymaga uwierzytelnienia, openctrlc poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `openctrlc mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji openctrlc bezpieczne przechowa tokeny w `~/.local/share/openctrlc/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w openctrlc, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie openctrlc, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć openctrlc z kontem Sentry.

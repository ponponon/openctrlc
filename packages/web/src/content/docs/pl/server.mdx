---
title: Serwer
description: Interakcja z serwerem openctrlc za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `openctrlc serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient openctrlc.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `openctrlc.local` |
...
Ustaw `OPENCTRLC_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `openctrlc` lub ustaw `OPENCTRLC_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `openctrlc serve`, jak i `openctrlc web`.
...
Kiedy uruchomisz `openctrlc`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera openctrlc do programowej interakcji z openctrlc.
:::

Ta architektura umożliwia obsługę wielu klientów przez openctrlc i programową interakcję z kodem otwartym.

Możesz uruchomić `openctrlc serve`, aby uruchomić samodzielny serwer. Jeśli masz
openctrlc TUI działa, `openctrlc serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki openctrlc [IDE](/docs/ide).
...
Serwer openctrlc udostępnia następujące interfejsy API.

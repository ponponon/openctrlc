...
Dzięki openctrlc możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie openctrlc używa naszego własnego motywu `openctrlc`.
...
openctrlc ma kilka wbudowanych motywów.
...
- Chcą, aby OpenCtrlC pasował do wyglądu ich terminala
...
openctrlc obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/openctrlc/themes/*.json` lub `$XDG_CONFIG_HOME/openctrlc/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.openctrlc/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.openctrlc/themes/*.json`

---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć openctrlc.
---

Wtyczki umożliwiające wyłączenie openctrlc poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania openctrlc.
...
- `.openctrlc/plugins/` - Wtyczki na poziomie projektu
- `~/.config/openctrlc/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/openctrlc/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/openctrlc/openctrlc.json`)
2. Konfiguracja projektu (`openctrlc.json`)
3. Globalny katalog wtyczek (`~/.config/openctrlc/plugins/`)
4. Katalog wtyczek projektu (`.openctrlc/plugins/`)
...
openctrlc uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z openctrlc do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do openctrlc.
...
:::note
Jeśli korzystasz z aplikacji komputerowej openctrlc, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj openctrlc czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do openctrlc:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod openctrlc. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://openctrlc.pages.dev/docs/sdk).

---
title: Server
description: Interager med openctrlc-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `openctrlc serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en openctrlc-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `openctrlc.local` |
...
Indstil `OPENCTRLC_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `openctrlc`, eller indstil `OPENCTRLC_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `openctrlc serve` og `openctrlc web`.
...
Når du kører `openctrlc` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug openctrlc-serveren til at interagere med openctrlc programmatisk.
:::

Denne arkitektur lader openctrlc understøtte flere klienter og lader dig interagere med openctrlc programmatisk.

Du kan køre `openctrlc serve` for at starte en selvstændig server. Hvis du har
openctrlc TUI kørende, vil `openctrlc serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af OpenCtrlC [IDE](/docs/ide) plugins.
...
OpenCtrlC-serveren viser følgende API'er.

...
OpenCode Desktop kører en lokal OpenCode-server (`openctrlc-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\opencode\openctrlc.jsonc`

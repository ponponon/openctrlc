...
Med OpenCtrlC kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger OpenCtrlC vårt eget `openctrlc`-tema.
...
OpenCtrlC støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/openctrlc/themes/*.json` eller `$XDG_CONFIG_HOME/openctrlc/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.openctrlc/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.openctrlc/themes/*.json`

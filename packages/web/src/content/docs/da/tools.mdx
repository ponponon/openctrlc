...
Her er alle de innebygde verktøyene som er tilgængelige i OpenCtrlC.
...
:::note
Dette verktøyet er bare tilgængelig når `OPENCTRLC_EXPERIMENTAL_LSP_TOOL=true` (eller `OPENCTRLC_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger OpenCtrlC-leverandøren eller når miljøvariabelen `OPENCTRLC_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).

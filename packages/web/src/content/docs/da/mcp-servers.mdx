...
Du kan tilføje eksterne værktøjer til OpenCtrlC ved hjælp af _Model Context Protocol_ eller MCP. OpenCtrlC understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [OpenCtrlC Config](https://openctrlc.pages.dev/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
OpenCtrlC håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil OpenCtrlC:
...
Hvis serveren kræver godkendelse, vil OpenCtrlC bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `openctrlc mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil OpenCtrlC opbevare tokens sikkert i `~/.local/share/openctrlc/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem OpenCtrlC-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde OpenCtrlC til din Sentry-konto.

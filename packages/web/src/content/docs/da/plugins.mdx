---
title: Plugins
description: Skriv dine egne plugins for at udvide OpenCtrlC.
---

Plugins giver dig mulighed for at udvide OpenCtrlC ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre OpenCtrlCs standardadfærd.
...
- `.openctrlc/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/openctrlc/node_modules/`.
...
1. Global konfiguration (`~/.config/openctrlc/openctrlc.json`)
2. Projektkonfiguration (`openctrlc.json`)
3. Global plugin-mappe (`~/.config/openctrlc/plugins/`)
4. Projekt plugin bibliotek (`.openctrlc/plugins/`)
...
OpenCtrlC kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En openctrlc SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide openctrlc.
...
:::note
Hvis du bruger OpenCtrlC desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå openctrlc i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til openctrlc:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som openctrlc kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for openctrlc sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://openctrlc.pages.dev/docs/sdk) for detaljer.

...
Agentferdigheter lar OpenCtrlC oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
OpenCtrlC søker etter disse stedene:

- Prosjektkonfigurasjon: `.openctrlc/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/openctrlc/skills/<name>/SKILL.md`
...
For prosjektlokale stier går OpenCtrlC opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.openctrlc/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/openctrlc/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.openctrlc/skills/git-release/SKILL.md` slik:
...
OpenCtrlC viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `openctrlc.json`:
...
**For innebygde agenter** (i `openctrlc.json`):

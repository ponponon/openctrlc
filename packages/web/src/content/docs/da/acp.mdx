...
description: Brug OpenCtrlC i enhver ACP-kompatibel editor.
---

OpenCtrlC understøtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), så du kan bruge det direkte i kompatible editorer og IDE'er.
...
For at bruge OpenCtrlC via ACP, konfigurer din editor til at køre kommandoen `openctrlc acp`.

Kommandoen starter OpenCtrlC som en ACP-kompatibel underproces, der kommunikerer med din editor over JSON-RPC via stdio.
...
For at åbne den skal du bruge den nye 'OpenCtrlC'-agent i AI Chat-agentvælgeren.
...
For at bruge OpenCtrlC som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), skal du tilføje følgende til din Neovim-konfiguration:
...
Denne konfiguration sætter CodeCompanion til at bruge OpenCtrlC som ACP-agent til chat.
...
OpenCtrlC fungerer på samme måde via ACP som i terminalen. Alle funktioner understøtter:
...
- MCP-servere konfigureret i din OpenCtrlC-konfiguration

...
Create `.openctrlc/commands/test.md`:
...
You can add custom commands through the OpenCtrlC config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your OpenCtrlC [config](/docs/config):
...
- Per-project: `.openctrlc/commands/`
...
openctrlc includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).

...
OpenCode is powered by the provider list at [Models.dev](https://models.dev), so you can use `openctrlc auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/openctrlc/auth.json`.
...
This starts an HTTP server that provides API access to opencode functionality without the TUI interface. Set `OPENCTRLC_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `openctrlc`).
...
This starts an HTTP server and opens a web browser to access OpenCode through a web interface. Set `OPENCTRLC_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `openctrlc`).

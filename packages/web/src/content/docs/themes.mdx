...
With OpenCtrlC you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, OpenCtrlC uses our own `openctrlc` theme.
...
OpenCtrlC comes with several built-in themes.
...
- Want OpenCtrlC to match their terminal's appearance
...
OpenCtrlC supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/openctrlc/themes/*.json` or `$XDG_CONFIG_HOME/openctrlc/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.openctrlc/themes/*.json`
4. **Current working directory** - Defined in `./.openctrlc/themes/*.json`

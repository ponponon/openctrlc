...
Las herramientas permiten que LLM realice acciones en su código base. OpenCtrlC viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en OpenCtrlC.
...
:::note
Esta herramienta solo está disponible cuando `OPENCTRLC_EXPERIMENTAL_LSP_TOOL=true` (o `OPENCTRLC_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor OpenCtrlC o cuando la variable de entorno `OPENCTRLC_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar OpenCtrlC:

```bash
OPENCTRLC_ENABLE_EXA=1 openctrlc
```

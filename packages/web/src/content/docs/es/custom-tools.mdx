...
description: Cree herramientas que LLM pueda llamar en openctrlc.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de openctrlc como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.openctrlc/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/openctrlc/tools/`.
...
```ts title=".openctrlc/tools/database.ts" {1}
...
```ts title=".openctrlc/tools/math.ts"
import { tool } from "@openctrlc/plugin"
...
```ts title=".openctrlc/tools/bash.ts"
import { tool } from "@openctrlc/plugin"
...
```ts title=".openctrlc/tools/project.ts" {8}
import { tool } from "@openctrlc/plugin"
...
```python title=".openctrlc/tools/add.py"
import sys
...
```ts title=".openctrlc/tools/python-add.ts" {10}
import { tool } from "@openctrlc/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".openctrlc/tools/add.py")

...
Puede agregar herramientas externas a OpenCtrlC utilizando el _Model Context Protocol_ o MCP. OpenCtrlC admite servidores locales y remotos.
...
Puede definir servidores MCP en su [OpenCtrlC Config](https://openctrlc.pages.dev/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
OpenCtrlC maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, OpenCtrlC:
...
Si el servidor requiere autenticación, OpenCtrlC le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `openctrlc mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, OpenCtrlC almacenará los tokens de forma segura en `~/.local/share/openctrlc/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en OpenCtrlC, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración OpenCtrlC como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar OpenCtrlC a su cuenta Sentry.

---
title: Servidor
description: Interactuar con el servidor openctrlc a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `openctrlc serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente openctrlc puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `openctrlc.local` |
...
Configure `OPENCTRLC_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `openctrlc`, o configure `OPENCTRLC_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `openctrlc serve` como a `openctrlc web`.
...
Cuando ejecuta `openctrlc`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor openctrlc para interactuar con openctrlc mediante programación.
:::

Esta arquitectura permite que openctrlc admita múltiples clientes y le permite interactuar con openctrlc mediante programación.

Puede ejecutar `openctrlc serve` para iniciar un servidor independiente. Si tienes el
openctrlc TUI ejecutándose, `openctrlc serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos OpenCtrlC [IDE](/docs/ide).
...
El servidor openctrlc expone las siguientes API.

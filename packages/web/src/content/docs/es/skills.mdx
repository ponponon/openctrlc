...
Las habilidades del agente permiten a OpenCtrlC descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
OpenCtrlC busca estas ubicaciones:

- Configuración del proyecto: `.openctrlc/skills/<name>/SKILL.md`
- Configuración global: `~/.config/openctrlc/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, OpenCtrlC sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.openctrlc/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/openctrlc/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.openctrlc/skills/git-release/SKILL.md` así:
...
OpenCtrlC enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `openctrlc.json`:
...
**Para agentes integrados** (en `openctrlc.json`):

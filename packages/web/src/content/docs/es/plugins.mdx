---
title: Complementos
description: Escriba sus propios complementos para extender OpenCtrlC.
---

Los complementos le permiten extender OpenCtrlC al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de OpenCtrlC.
...
- `.openctrlc/plugins/` - Complementos a nivel de proyecto
- `~/.config/openctrlc/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/openctrlc/node_modules/`.
...
1. Configuración global (`~/.config/openctrlc/openctrlc.json`)
2. Configuración del proyecto (`openctrlc.json`)
3. Directorio global de complementos (`~/.config/openctrlc/plugins/`)
4. Directorio de complementos del proyecto (`.openctrlc/plugins/`)
...
```json title=".openctrlc/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
OpenCtrlC ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".openctrlc/plugins/my-plugin.ts"
...
```js title=".openctrlc/plugins/example.js"
...
- `client`: Un cliente SDK openctrlc para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar openctrlc.
...
```js title=".openctrlc/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "openctrlc"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio OpenCtrlC, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que openctrlc lea archivos `.env`:

```javascript title=".openctrlc/plugins/env-protection.js"
...
```javascript title=".openctrlc/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a openctrlc:

```ts title=".openctrlc/plugins/custom-tools.ts"
import { type Plugin, tool } from "@openctrlc/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que openctrlc puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para openctrlc junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://openctrlc.pages.dev/docs/sdk) para obtener más detalles.
...
```ts title=".openctrlc/plugins/compaction.ts"
import type { Plugin } from "@openctrlc/plugin"
...
```ts title=".openctrlc/plugins/custom-compaction.ts"
import type { Plugin } from "@openctrlc/plugin"

...
Para depurar problemas con OpenCtrlC, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/openctrlc/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\openctrlc\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `openctrlc --log-level DEBUG`.

---

## Almacenamiento

openctrlc almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/openctrlc/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\openctrlc`
...
OpenCtrlC Desktop ejecuta un servidor OpenCtrlC local (el sidecar `openctrlc-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `OpenCtrlC` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/openctrlc/openctrlc.jsonc` (o `~/.config/openctrlc/openctrlc.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/openctrlc/openctrlc.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\openctrlc\openctrlc.jsonc`
...
OpenCtrlC también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/openctrlc/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\openctrlc\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.openctrlc/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que OpenCtrlC pueda reconstruirlo.

1. Salga de OpenCtrlC Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/openctrlc`
- **Linux**: eliminar `~/.cache/openctrlc` (o ejecutar `rm -rf ~/.cache/openctrlc`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\openctrlc`

3. Reinicie el escritorio OpenCtrlC.

---

### Solucionar problemas de conexión del servidor

OpenCtrlC Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `openctrlc.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `OPENCTRLC_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `OPENCTRLC_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio OpenCtrlC requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de OpenCtrlC.

---

### Notificaciones que no se muestran

OpenCtrlC Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para OpenCtrlC en la configuración de su sistema operativo, y
...
1. Salga del escritorio OpenCtrlC.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio OpenCtrlC):

- `openctrlc.settings.dat` (URL del servidor predeterminado de escritorio)
- `openctrlc.global.dat` y `openctrlc.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con OpenCtrlC:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/ponponon/openctrlc/issues**](https://github.com/ponponon/openctrlc/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**github.com/ponponon/openctrlc/discussions**](https://github.com/ponponon/openctrlc/discussions)
...
3. Asegúrese de tener la última versión con `openctrlc upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `openctrlc models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/openctrlc
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\openctrlc`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. openctrlc instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   ```bash
   rm -rf ~/.cache/openctrlc
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\openctrlc`

2. Reinicie openctrlc para reinstalar los paquetes del proveedor más recientes.

Esto obligará a openctrlc a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
openctrlc detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.

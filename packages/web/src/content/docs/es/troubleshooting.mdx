...
OpenCode Desktop ejecuta un servidor OpenCode local (el sidecar `openctrlc-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\opencode\openctrlc.jsonc`

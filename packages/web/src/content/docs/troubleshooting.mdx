...
OpenCode Desktop runs a local OpenCode server (the `openctrlc-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.
...
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\opencode\openctrlc.jsonc`

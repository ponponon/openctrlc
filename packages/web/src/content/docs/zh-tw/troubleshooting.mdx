...
OpenCode Desktop 會在背景執行一個本地 OpenCode 伺服器（即 `openctrlc-cli` 附屬程序）。大多數問題是由外掛異常、快取損壞或錯誤的伺服器設定引起的。
...
- **Windows**: 按 `WIN+R` 並貼上 `%USERPROFILE%\.config\opencode\openctrlc.jsonc`

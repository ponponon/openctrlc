---
title: LSP Servers
description: OpenCtrlC integrates with your LSP servers.
---

OpenCtrlC can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

OpenCtrlC comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `OPENCTRLC_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and openctrlc opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your openctrlc config.
...
```json title="openctrlc.json" {5-13}
{
  "$schema": "https://openctrlc.pages.dev/config.json",
  "lsp": {
    "custom-lsp": {

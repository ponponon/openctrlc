...
Создайте `.openctrlc/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию openctrlc или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем openctrlc [config](/docs/config):
...
- Глобальный: `~/.config/openctrlc/commands/`
- Для каждого проекта: `.openctrlc/commands/`
...
openctrlc включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).

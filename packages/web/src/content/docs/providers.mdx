...
OpenCtrlC does not bundle a hosted model service. For lightweight tasks such as
generating session titles, configure `small_model` explicitly if you want to
control which provider is used. To keep these tasks on your self-hosted GitLab
instance, add the following to `openctrlc.json`.

...
The `openctrlc serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an opencode client can use.
...
:::tip
Use the openctrlc server to interact with opencode programmatically.
:::
...
opencode TUI running, `openctrlc serve` will start a new server.

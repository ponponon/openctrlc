---
title: Server
description: Interact with openctrlc server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `openctrlc serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an openctrlc client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `openctrlc.local` |
...
Set `OPENCTRLC_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `openctrlc`, or set `OPENCTRLC_SERVER_USERNAME` to override it. This applies to both `openctrlc serve` and `openctrlc web`.
...
When you run `openctrlc` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the openctrlc server to interact with openctrlc programmatically.
:::

This architecture lets openctrlc support multiple clients and allows you to interact with openctrlc programmatically.

You can run `openctrlc serve` to start a standalone server. If you have the
openctrlc TUI running, `openctrlc serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the OpenCtrlC [IDE](/docs/ide) plugins.
...
The openctrlc server exposes the following APIs.

...
Avec OpenCtrlC, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, OpenCtrlC utilise notre propre thème `openctrlc`.
...
OpenCtrlC est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que OpenCtrlC corresponde à l'apparence de leur terminal
...
OpenCtrlC prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/openctrlc/themes/*.json` ou `$XDG_CONFIG_HOME/openctrlc/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.openctrlc/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.openctrlc/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/openctrlc/themes
vim ~/.config/openctrlc/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .openctrlc/themes
vim .openctrlc/themes/my-theme.json
```
...
{
  "$schema": "https://openctrlc.pages.dev/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",

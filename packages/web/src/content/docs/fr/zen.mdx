---
title: Zen
description: Liste organisée de modèles fournis par OpenCtrlC.
---
...
OpenCtrlC Zen est une liste de modèles testés et vérifiés fournie par l'équipe OpenCtrlC.

Zen fonctionne comme n'importe quel autre fournisseur dans OpenCtrlC. Vous vous connectez à OpenCtrlC Zen et obtenez votre clé API. C'est **entièrement facultatif** et vous n'avez pas besoin de l'utiliser pour utiliser OpenCtrlC.
...
:::tip
Nous avons testé un groupe sélectionné de modèles et de fournisseurs qui fonctionnent bien avec OpenCtrlC.
:::
...
OpenCtrlC Zen est une passerelle AI qui vous donne accès à ces modèles.

---

## Comment ça marche

OpenCtrlC Zen fonctionne comme n'importe quel autre fournisseur dans OpenCtrlC.

1. Vous vous connectez à **<a href={console}>OpenCtrlC Zen</a>**, ajoutez vos informations de facturation et copiez votre clé API.
2. Vous exécutez la commande `/connect` dans le TUI, sélectionnez OpenCtrlC Zen et collez votre clé API.
...
Le [model id](/docs/config/#models) dans votre configuration OpenCtrlC utilise le format `openctrlc/<model-id>`. Par exemple, pour GPT 5.5, vous utiliseriez `opencode/gpt-5.5` dans votre configuration.
...
Vous remarquerez peut-être des [modèles à faible coût](/docs/config/#models), tels que Haiku, Nano ou Flash, dans votre historique d'utilisation. OpenCtrlC utilise ces modèles pour générer les titres des sessions.
...
- DeepSeek V4 Flash Free est disponible sur OpenCtrlC pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- MiMo-V2.5 Free est disponible sur OpenCtrlC pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Hy3 Free est disponible sur OpenCtrlC pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Laguna S 2.1 Free est disponible sur OpenCtrlC pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Nemotron 3 Ultra Free est disponible sur OpenCtrlC pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Nemotron 3.5 Lightning Free est disponible sur OpenCtrlC pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Big Pickle est un modèle stealth gratuit sur OpenCtrlC pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
...
Nous avons créé OpenCtrlC Zen pour :
...
4. N'avoir **aucun lock-in** en vous permettant de l'utiliser avec n'importe quel autre agent de codage. Et toujours vous permettre d'utiliser n'importe quel autre fournisseur avec OpenCtrlC également.

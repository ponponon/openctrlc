---
title: Plugins
description: Écrivez vos propres plugins pour étendre OpenCtrlC.
---

Les plugins vous permettent d'étendre OpenCtrlC en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de OpenCtrlC.
...
- `.openctrlc/plugins/` - Plugins au niveau du projet
- `~/.config/openctrlc/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/openctrlc/node_modules/`.
...
1. Configuration globale (`~/.config/openctrlc/openctrlc.json`)
2. Configuration du projet (`openctrlc.json`)
3. Répertoire global des plugins (`~/.config/openctrlc/plugins/`)
4. Répertoire des plugins du projet (`.openctrlc/plugins/`)
...
OpenCtrlC exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK openctrlc pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre openctrlc.
...
:::note
Si vous utilisez l'application de bureau OpenCtrlC, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez openctrlc de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à openctrlc :
...
L'assistant `tool` crée un outil personnalisé que openctrlc peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour openctrlc aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://openctrlc.pages.dev/docs/sdk) pour plus de détails.

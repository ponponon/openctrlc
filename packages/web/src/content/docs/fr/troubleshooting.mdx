...
OpenCode Desktop exécute un serveur OpenCode local (le side-car `openctrlc-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\opencode\openctrlc.jsonc`

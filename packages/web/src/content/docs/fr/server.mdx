...
La commande `openctrlc serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client opencode peut utiliser.
...
opencode TUI en cours d'exécution, `openctrlc serve` démarrera un nouveau serveur.

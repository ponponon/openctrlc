---
title: Serveur
description: Interagissez avec le serveur openctrlc via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `openctrlc serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client openctrlc peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `openctrlc.local` |
...
Définissez `OPENCTRLC_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `openctrlc`, ou définissez `OPENCTRLC_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `openctrlc serve` et à `openctrlc web`.
...
Lorsque vous exécutez `openctrlc`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur openctrlc pour interagir avec openctrlc par programmation.
:::

Cette architecture permet à openctrlc de prendre en charge plusieurs clients et vous permet d'interagir avec openctrlc par programme.

Vous pouvez exécuter `openctrlc serve` pour démarrer un serveur autonome. Si vous avez le
openctrlc TUI en cours d'exécution, `openctrlc serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins OpenCtrlC [IDE](/docs/ide).
...
Le serveur openctrlc expose les API suivantes.

---
title: Support ACP
description: Utilisez OpenCtrlC dans n’importe quel éditeur compatible ACP.
---

OpenCtrlC prend en charge le [Agent Client Protocol](https://agentclientprotocol.com) (ACP), vous permettant de l'utiliser directement dans les éditeurs et IDE compatibles.
...
Pour utiliser OpenCtrlC via ACP, configurez votre éditeur pour exécuter la commande `openctrlc acp`.

La commande démarre OpenCtrlC en tant que sous-processus compatible ACP qui communique avec votre éditeur via JSON-RPC sur stdio.
...
```json title="acp.json"
{
  "agent_servers": {
    "OpenCtrlC": {
      "command": "/absolute/path/bin/openctrlc",
      "args": ["acp"]
    }
  }
}
...
Pour l'ouvrir, utilisez le nouvel agent « OpenCtrlC » dans le sélecteur d'agent AI Chat.
...
Pour utiliser OpenCtrlC comme agent ACP dans [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), ajoutez ce qui suit à votre configuration Neovim :
...
Cette configuration configure CodeCompanion pour utiliser OpenCtrlC comme agent ACP pour le chat.
...
OpenCtrlC fonctionne de la même manière via ACP que dans le terminal. Toutes les fonctionnalités sont prises en charge :
...
- Serveurs MCP configurés dans votre configuration OpenCtrlC

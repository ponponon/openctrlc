...
Tools allow the LLM to perform actions in your codebase. OpenCtrlC comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in OpenCtrlC.
...
:::note
This tool is only available when `OPENCTRLC_EXPERIMENTAL_LSP_TOOL=true` (or `OPENCTRLC_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using OpenCtrlC with a configured provider, or when the `OPENCTRLC_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching OpenCtrlC:

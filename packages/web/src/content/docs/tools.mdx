...
:::note
This tool is only available when using the OpenCtrlC or OpenCtrlC Go provider, or when the `OPENCTRLC_ENABLE_EXA` environment variable is set to any truthy value (e.g., `true` or `1`).

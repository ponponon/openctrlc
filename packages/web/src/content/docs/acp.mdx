---
title: ACP Support
description: Use OpenCtrlC in any ACP-compatible editor.
---

OpenCtrlC supports the [Agent Client Protocol](https://agentclientprotocol.com) or (ACP), allowing you to use it directly in compatible editors and IDEs.
...
To use OpenCtrlC via ACP, configure your editor to run the `openctrlc acp` command.

The command starts OpenCtrlC as an ACP-compatible subprocess that communicates with your editor over JSON-RPC via stdio.
...
To open it, use the new 'OpenCtrlC' agent in the AI Chat agent selector.
...
To use OpenCtrlC as an ACP agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), add the following to your Neovim config:
...
This config sets up CodeCompanion to use OpenCtrlC as the ACP agent for chat.
...
OpenCtrlC works the same via ACP as it does in the terminal. All features are supported:
...
- MCP servers configured in your OpenCtrlC config

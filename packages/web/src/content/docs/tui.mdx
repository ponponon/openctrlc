---
title: TUI
description: Using the OpenCtrlC terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenCtrlC provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running OpenCtrlC starts the TUI for the current directory.
...
When using the OpenCtrlC TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to OpenCtrlC. Allows you to select from available providers and add their API keys.
...
Exit OpenCtrlC. _Aliases_: `/quit`, `/q`
...
This is separate from `openctrlc.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long OpenCtrlC waits after the leader key. Defaults to `2000`.
...
Use `OPENCTRLC_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `openctrlc.default`.

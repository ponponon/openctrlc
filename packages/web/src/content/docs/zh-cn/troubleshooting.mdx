...
OpenCode Desktop 会在后台运行一个本地 OpenCode 服务器（即 `openctrlc-cli` 附属进程）。大多数问题是由插件异常、缓存损坏或错误的服务器设置引起的。
...
- **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.config\opencode\openctrlc.jsonc`

---
title: プラグイン
description: OpenCtrlC を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで OpenCtrlC を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、OpenCtrlC のデフォルトの動作を変更したりできます。
...
- `.openctrlc/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/openctrlc/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/openctrlc/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/openctrlc/openctrlc.json`)
2. プロジェクト設定 (`openctrlc.json`)
3. グローバルプラグインディレクトリ (`~/.config/openctrlc/plugins/`)
4. プロジェクトプラグインディレクトリ (`.openctrlc/plugins/`)
...
OpenCtrlC は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための OpenCtrlC SDK クライアント。
...
ここでは、OpenCtrlC を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://openctrlc.pages.dev/docs/sdk) を参照してください。
...
OpenCtrlC が `.env` ファイルを読み取らないようにします。
...
プラグインは OpenCtrlC にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、OpenCtrlC が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して OpenCtrlC で使用できます。

---
title: OpenCtrlC Zen
description: OpenCtrlC によって提供されるモデルの厳選されたリスト。
---
...
OpenCtrlC Zen は、OpenCtrlC チームが提供する、テスト済みかつ検証済みのモデルの一覧です。

Zen は OpenCtrlC のほかのプロバイダーと同じように動作します。OpenCtrlC Zen にログインして API キーを取得できます。これは **完全にオプション** であり、OpenCtrlC を使うために必須ではありません。
...
:::tip
OpenCtrlC でうまく動作する、厳選したモデルとプロバイダーの組み合わせをテストしました。
:::
...
OpenCtrlC Zen は、これらのモデルにアクセスできる AI ゲートウェイです。

---

## 仕組み

OpenCtrlC Zen は、OpenCtrlC のほかのプロバイダーと同じように動作します。

1. **<a href={console}>OpenCtrlC Zen</a>** にサインインし、請求情報を追加して、API キーをコピーします。
2. TUI で `/connect` コマンドを実行し、OpenCtrlC Zen を選んで API キーを貼り付けます。
...
OpenCtrlC 設定で使う [model id](/docs/config/#models) は `openctrlc/<model-id>` 形式です。たとえば、GPT 5.5 では設定に `opencode/gpt-5.5` を使用します。
...
使用履歴に Haiku、Nano、Flash などの[低コストモデル](/docs/config/#models)が表示されることがあります。OpenCtrlC はこれらのモデルをセッションタイトルの生成に使用します。
...
- DeepSeek V4 Flash Free は期間限定で OpenCtrlC で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- MiMo-V2.5 Free は期間限定で OpenCtrlC で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Hy3 Free は期間限定で OpenCtrlC で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Laguna S 2.1 Free は期間限定で OpenCtrlC で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Nemotron 3 Ultra Free は期間限定で OpenCtrlC で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Nemotron 3.5 Lightning Free は期間限定で OpenCtrlC で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Big Pickle はステルスモデルで、期間限定で OpenCtrlC で無料提供されています。チームはこの期間中にフィードバックを集め、モデルを改善しています。
...
私たちが OpenCtrlC Zen を作った目的は次のとおりです。
...
4. ほかのコーディングエージェントでも使えるようにして **no lock-in** を実現し、さらに OpenCtrlC では常にほかのプロバイダーも使えるようにすること。

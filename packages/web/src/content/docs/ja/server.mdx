---
title: サーバー
description: HTTP 経由で OpenCtrlC サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`openctrlc serve` コマンドは、OpenCtrlC クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `openctrlc.local` |
...
HTTP 基本認証でサーバーを保護するには、`OPENCTRLC_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `openctrlc` になるか、`OPENCTRLC_SERVER_USERNAME` を設定してオーバーライドします。これは、`openctrlc serve` と `openctrlc web` の両方に当てはまります。
...
`openctrlc` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
OpenCtrlC サーバーを使用して、プログラムで OpenCtrlC と対話します。
:::

`openctrlc serve` を実行してスタンドアロンサーバーを起動できます。すでに OpenCtrlC TUI を実行している場合でも、`openctrlc serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、OpenCtrlC [IDE](/docs/ide) プラグインによって使用されます。

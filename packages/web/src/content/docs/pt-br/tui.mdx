---
title: TUI
description: Usando a interface de usuário de terminal openctrlc.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

O openctrlc fornece uma interface de terminal interativa ou TUI para trabalhar em seus projetos com um LLM.

Executar openctrlc inicia o TUI para o diretório atual.
...
```text "@src/api/index.ts"
Como a autenticação é tratada em @src/api/index.ts?
```
...
Ao usar o TUI do openctrlc, você pode digitar `/` seguido pelo nome de um comando para executar ações rapidamente. Por exemplo:
...
Adicione um provedor ao openctrlc. Permite que você selecione entre os provedores disponíveis e adicione suas chaves de API.
...
Saia do openctrlc. _Aliases_: `/quit`, `/q`
...
Isso é separado do `openctrlc.json`, que configura o comportamento do servidor/runtime.
...
- `leader_timeout` - Controla por quanto tempo o OpenCtrlC espera depois da leader key. O padrão é `2000`.
...
Use `OPENCTRLC_TUI_CONFIG` para carregar um caminho de configuração TUI personalizado.

### Attention

Attention permite que o TUI avise você quando o OpenCtrlC espera uma resposta, precisa da aprovação de uma permissão, encontra um erro de sessão ou conclui uma sessão. Ative com `attention.enabled`; eventos integrados reproduzem sons quando acontecem. As notificações de desktop só são enviadas quando a janela do terminal não está em foco e não são usadas para eventos de subagent.
...
- `sound_pack` - ID do sound pack a ser usado. O padrão é `openctrlc.default`.

...
O opencode Desktop executa um servidor opencode local (o sidecar `openctrlc-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\opencode\openctrlc.jsonc`

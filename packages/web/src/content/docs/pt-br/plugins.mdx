---
title: Plugins
description: Escreva seus próprios plugins para estender o openctrlc.
---

Plugins permitem que você estenda o openctrlc conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do openctrlc.
...
- `.openctrlc/plugins/` - Plugins em nível de projeto
- `~/.config/openctrlc/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/openctrlc/node_modules/`.
...
1. Configuração global (`~/.config/openctrlc/openctrlc.json`)
2. Configuração do projeto (`openctrlc.json`)
3. Diretório de plugins global (`~/.config/openctrlc/plugins/`)
4. Diretório de plugins do projeto (`.openctrlc/plugins/`)
...
O openctrlc executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do openctrlc para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o openctrlc.
...
:::note
Se você estiver usando o aplicativo desktop openctrlc, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o openctrlc de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao openctrlc:
...
O helper `tool` cria uma ferramenta personalizada que o openctrlc pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o openctrlc junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://openctrlc.pages.dev/docs/sdk) para detalhes.

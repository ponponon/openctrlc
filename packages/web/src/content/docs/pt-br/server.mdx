...
O comando `openctrlc serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente opencode pode usar.
...
Você pode executar `openctrlc serve` para iniciar um servidor autônomo. Se você tiver o TUI do opencode em execução, `openctrlc serve` iniciará um novo servidor.

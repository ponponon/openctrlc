---
title: Servidor
description: Interaja com o servidor openctrlc via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `openctrlc serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente openctrlc pode usar.
...
```bash
openctrlc serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `openctrlc.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`              |

`--cors` pode ser passado várias vezes:

```bash
openctrlc serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OPENCTRLC_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `openctrlc`, ou defina `OPENCTRLC_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `openctrlc serve` quanto ao `openctrlc web`.

```bash
OPENCTRLC_SERVER_PASSWORD=your-password openctrlc serve
```
...
Quando você executa `openctrlc`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor openctrlc para interagir com o openctrlc programaticamente.
:::

Essa arquitetura permite que o openctrlc suporte múltiplos clientes e permite que você interaja com o openctrlc programaticamente.

Você pode executar `openctrlc serve` para iniciar um servidor autônomo. Se você tiver o TUI do openctrlc em execução, `openctrlc serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do openctrlc [IDE](/docs/ide).
...
O servidor openctrlc expõe as seguintes APIs.

...
![openctrlc TUI com o tema openctrlc](../../../assets/lander/screenshot.png)
...
A maneira mais fácil de instalar o openctrlc é através do script de instalação.
...
- **Usando Homebrew no macOS e Linux**
  ...
  > Recomendamos usar o tap do openctrlc para as versões mais atualizadas. A fórmula oficial `brew install openctrlc` é mantida pela equipe do Homebrew e é atualizada com menos frequência.
...
Para a melhor experiência no Windows, recomendamos usar o [Windows Subsystem for Linux (WSL)](/docs/windows-wsl). Ele oferece melhor desempenho e total compatibilidade com os recursos do openctrlc.
:::
...
O suporte para instalar o openctrlc no Windows usando Bun está atualmente em andamento.

Você também pode baixar o binário na seção [Releases](https://github.com/ponponon/openctrlc/releases).

---

## Configuração

Com o openctrlc, você pode usar qualquer provedor de LLM configurando suas chaves de API.

Se você é novo no uso de provedores de LLM, recomendamos usar o [OpenCtrlC Zen](/docs/zen).
É uma lista selecionada de modelos que foram testados e verificados pela equipe do openctrlc.

1. Execute o comando `/connect` no TUI, selecione openctrlc e acesse [openctrlc.ai/auth](https://opencode.ai/auth).
...
Aqui está uma [conversa de exemplo](https://opencode.ai/s/4XP1fce5) com o openctrlc.

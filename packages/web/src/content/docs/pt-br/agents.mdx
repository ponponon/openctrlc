...
Existem dois tipos de agentes no openctrlc; agentes primários e subagentes.
...
openctrlc vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
OpenCtrlC vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

OpenCtrlC vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do OpenCtrlC, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `openctrlc.json`:
...
- Global: `~/.config/openctrlc/agents/`
- Por projeto: `.openctrlc/agents/`

```markdown title="~/.config/openctrlc/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o openctrlc usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do openctrlc quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do openctrlc usa o formato `provider/model-id`. Por exemplo, se você estiver usando [seu provedor configurado](/docs/providers), você usaria `opencode/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="openctrlc.json" {3-6,9-12}
{
  "$schema": "https://openctrlc.pages.dev/config.json",
  "tools": {
    "write": true,
...
```json title="openctrlc.json" {3-5,8-10}
{
  "$schema": "https://openctrlc.pages.dev/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `openctrlc models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/ponponon/openctrlc).
:::
...
```markdown title="~/.config/openctrlc/agents/docs-writer.md"
---
...
```markdown title="~/.config/openctrlc/agents/security-auditor.md"
---

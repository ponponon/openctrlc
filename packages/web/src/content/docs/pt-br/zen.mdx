---
title: Zen
description: Lista selecionada de modelos fornecidos pelo OpenCtrlC.
---
...
O OpenCtrlC Zen é uma lista de modelos testados e verificados fornecidos pela equipe do OpenCtrlC.

O Zen funciona como qualquer outro provedor no OpenCtrlC. Você faz login no OpenCtrlC Zen e recebe sua chave de API. É **completamente opcional** e você não precisa usá-lo para usar o OpenCtrlC.
...
:::tip
Testamos um grupo selecionado de modelos e provedores que funcionam bem com o OpenCtrlC.
:::
...
O OpenCtrlC Zen é um gateway de AI que dá a você acesso a esses modelos.

---

## Como funciona

O OpenCtrlC Zen funciona como qualquer outro provedor no OpenCtrlC.

1. Você faz login no **<a href={console}>OpenCtrlC Zen</a>**, adiciona seus dados de cobrança e copia sua chave de API.
2. Você executa o comando `/connect` no TUI, seleciona OpenCtrlC Zen e cola sua chave de API.
...
O [model id](/docs/config/#models) na sua configuração do OpenCtrlC usa o formato `openctrlc/<model-id>`. Por exemplo, para GPT 5.5, você usaria `opencode/gpt-5.5` na sua configuração.
...
Você pode notar [modelos de baixo custo](/docs/config/#models), como Haiku, Nano ou Flash, no seu histórico de uso. O OpenCtrlC usa esses modelos para gerar títulos de sessões.
...
- DeepSeek V4 Flash Free está disponível no OpenCtrlC por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- MiMo-V2.5 Free está disponível no OpenCtrlC por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Hy3 Free está disponível no OpenCtrlC por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Laguna S 2.1 Free está disponível no OpenCtrlC por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Nemotron 3 Ultra Free está disponível no OpenCtrlC por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Nemotron 3.5 Lightning Free está disponível no OpenCtrlC por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
- Big Pickle é um modelo stealth que está gratuito no OpenCtrlC por tempo limitado. A equipe está usando esse período para coletar feedback e melhorar o modelo.
...
4. Não ter **lock-in**, permitindo que você o use com qualquer outro agente de codificação. E sempre permitir que você use qualquer outro provedor com o OpenCtrlC também.

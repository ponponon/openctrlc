...
Você pode adicionar ferramentas externas ao openctrlc usando o _Model Context Protocol_, ou MCP. O openctrlc suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do openctrlc](https://openctrlc.pages.dev/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
O openctrlc lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o openctrlc irá:
...
Se o servidor requer autenticação, o openctrlc solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `openctrlc mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o openctrlc armazenará os tokens de forma segura em `~/.local/share/openctrlc/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no openctrlc, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do openctrlc como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o openctrlc à sua conta do Sentry.

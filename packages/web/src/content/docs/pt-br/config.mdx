...
2. **Configuração global** (`~/.config/opencode/openctrlc.json`) - preferências do usuário
...
:::note
Os diretórios `.openctrlc` e `~/.config/opencode` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
Coloque sua configuração global do opencode em `~/.config/opencode/openctrlc.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/opencode/agents/` ou `.openctrlc/agents/`. [Saiba mais aqui](/docs/agents).
...
Você também pode definir comandos usando arquivos markdown em `~/.config/opencode/commands/` ou `.openctrlc/commands/`. [Saiba mais aqui](/docs/commands).
...
Coloque arquivos de plugin em `.openctrlc/plugins/` ou `~/.config/opencode/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.

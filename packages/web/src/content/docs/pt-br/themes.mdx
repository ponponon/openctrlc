...
Com o openctrlc, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o openctrlc usa nosso próprio tema `openctrlc`.
...
O openctrlc vem com vários temas embutidos.
...
- Querem que o openctrlc corresponda à aparência do seu terminal
...
O openctrlc suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/openctrlc/themes/*.json` ou `$XDG_CONFIG_HOME/openctrlc/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.openctrlc/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.openctrlc/themes/*.json`

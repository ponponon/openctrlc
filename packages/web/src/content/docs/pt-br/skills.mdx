...
As habilidades do agente permitem que o openctrlc descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O openctrlc pesquisa nesses locais:

- Configuração do projeto: `.openctrlc/skills/<name>/SKILL.md`
- Configuração global: `~/.config/openctrlc/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o openctrlc sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.openctrlc/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/openctrlc/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.openctrlc/skills/git-release/SKILL.md` assim:
...
O openctrlc lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `openctrlc.json`:
...
**Para agentes embutidos** (em `openctrlc.json`):

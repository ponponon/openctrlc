...
أنشئ الملف `.openctrlc/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات OpenCtrlC أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ OpenCtrlC:
...
- على مستوى النظام: `~/.config/openctrlc/commands/`
- لكل مشروع: `.openctrlc/commands/`
...
يتضمن openctrlc عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).

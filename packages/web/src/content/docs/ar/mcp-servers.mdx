...
يمكنك إضافة أدوات خارجية إلى OpenCtrlC باستخدام _Model Context Protocol_، أو MCP. يدعم OpenCtrlC خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات OpenCtrlC](https://openctrlc.pages.dev/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يتولى OpenCtrlC تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم OpenCtrlC بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك OpenCtrlC المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `openctrlc mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن OpenCtrlC الرموز بشكل آمن في `~/.local/share/openctrlc/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل OpenCtrlC إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات OpenCtrlC مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط OpenCtrlC بحساب Sentry الخاص بك.

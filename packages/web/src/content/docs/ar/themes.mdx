...
مع OpenCtrlC يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم OpenCtrlC سمتنا `openctrlc`.
...
يأتي OpenCtrlC مع عدة سمات مدمجة.
...
- يريدون أن يطابق OpenCtrlC مظهر terminal لديهم
...
يدعم OpenCtrlC نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/openctrlc/themes/*.json` أو `$XDG_CONFIG_HOME/openctrlc/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.openctrlc/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.openctrlc/themes/*.json`

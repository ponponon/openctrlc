...
In OpenCtrlC esistono due tipi di agenti: agenti primari e subagenti.
...
OpenCtrlC include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
OpenCtrlC include tre subagenti integrati: **General**, **Explore** e **Scout**. Li vediamo sotto.

---

## Integrati

OpenCtrlC include due agenti primari integrati e tre subagenti integrati.
...
Un agente in sola lettura per la ricerca su documentazione esterna e dipendenze. Usalo quando devi clonare il repository di una dipendenza nella cache gestita di OpenCtrlC, ispezionare il codice sorgente di una libreria o confrontare il codice locale con implementazioni upstream senza modificare il tuo workspace.
...
Configura gli agenti nel file `openctrlc.json`:
...
- Globale: `~/.config/openctrlc/agents/`
- Per progetto: `.openctrlc/agents/`
...
Se non specifichi una temperature, OpenCtrlC usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di OpenCtrlC sia per la config specifica del progetto.
...
L'ID modello nella configurazione OpenCtrlC usa il formato `provider/model-id`. Per esempio, se stai usando [il provider configurato](/docs/providers), useresti `opencode/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `openctrlc models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/ponponon/openctrlc).
:::

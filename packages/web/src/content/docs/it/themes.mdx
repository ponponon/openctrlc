...
Con OpenCtrlC puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, OpenCtrlC usa il tema `openctrlc`.
...
OpenCtrlC include diversi temi integrati.
...
- Vuole che OpenCtrlC corrisponda all'aspetto del terminale
...
OpenCtrlC supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/openctrlc/themes/*.json` o `$XDG_CONFIG_HOME/openctrlc/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.openctrlc/themes/*.json`
4. **Directory di lavoro corrente** - in `./.openctrlc/themes/*.json`

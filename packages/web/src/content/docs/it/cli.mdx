...
OpenCode si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `openctrlc auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/openctrlc/auth.json`.
...
Avvia un server HTTP che espone accesso API alle funzionalità di opencode senza la TUI. Imposta `OPENCTRLC_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `openctrlc`).
...
Avvia un server HTTP e apre un browser per accedere a OpenCode tramite interfaccia web. Imposta `OPENCTRLC_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `openctrlc`).

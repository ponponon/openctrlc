...
OpenCode Desktop esegue in background un server locale di OpenCode (il sidecar `openctrlc-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\opencode\openctrlc.jsonc`

---
title: Supporto ACP
description: Usa OpenCtrlC in qualunque editor compatibile con ACP.
---

OpenCtrlC supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare OpenCtrlC via ACP, configura l'editor in modo che esegua il comando `openctrlc acp`.

Il comando avvia OpenCtrlC come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'OpenCtrlC' nel selettore agenti di AI Chat.
...
Per usare OpenCtrlC come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare OpenCtrlC come agente ACP per la chat.
...
OpenCtrlC funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione OpenCtrlC

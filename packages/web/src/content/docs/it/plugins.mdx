---
title: Plugin
description: Scrivi plugin per estendere OpenCtrlC.
---

I plugin ti permettono di estendere OpenCtrlC agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di OpenCtrlC.
...
- `.openctrlc/plugins/` - plugin a livello progetto
- `~/.config/openctrlc/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/openctrlc/node_modules/`.
...
1. Config globale (`~/.config/openctrlc/openctrlc.json`)
2. Config di progetto (`openctrlc.json`)
3. Directory plugin globale (`~/.config/openctrlc/plugins/`)
4. Directory plugin di progetto (`.openctrlc/plugins/`)
...
OpenCtrlC esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di openctrlc per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere openctrlc.
...
:::note
Se usi l'app desktop di OpenCtrlC, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a openctrlc di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a openctrlc:
...
L'helper `tool` crea uno strumento personalizzato che openctrlc può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in openctrlc insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://openctrlc.pages.dev/docs/sdk) per i dettagli.

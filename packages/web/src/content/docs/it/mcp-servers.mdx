...
Puoi aggiungere strumenti esterni a OpenCtrlC usando il _Model Context Protocol_, o MCP. OpenCtrlC supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [OpenCtrlC Config](https://openctrlc.pages.dev/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
OpenCtrlC gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, OpenCtrlC:
...
Se il server richiede autenticazione, OpenCtrlC ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `openctrlc mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, OpenCtrlC memorizzera' i token in modo sicuro in `~/.local/share/openctrlc/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in OpenCtrlC insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione OpenCtrlC come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare OpenCtrlC al tuo account Sentry.

...
Le skill degli agenti permettono a OpenCtrlC di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
OpenCtrlC cerca in queste posizioni:

- Config di progetto: `.openctrlc/skills/<name>/SKILL.md`
- Config globale: `~/.config/openctrlc/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, OpenCtrlC risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.openctrlc/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/openctrlc/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.openctrlc/skills/git-release/SKILL.md` cosi':
...
OpenCtrlC elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `openctrlc.json`:
...
**Per agenti integrati** (in `openctrlc.json`):

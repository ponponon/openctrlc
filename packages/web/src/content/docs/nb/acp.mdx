---
title: ACP-støtte
description: Bruk openctrlc i en hvilken som helst ACP-kompatibel editor.
---

OpenCtrlC støtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), slik at du kan bruke den direkte i kompatible editorer og IDE-er.
...
For å bruke OpenCtrlC via ACP, konfigurer redigeringsprogrammet til å kjøre kommandoen `openctrlc acp`.

Kommandoen starter OpenCtrlC som en ACP-kompatibel underprosess som kommuniserer med editoren din over JSON-RPC via stdio.
...
For å åpne den, bruk den nye 'OpenCtrlC'-agenten i AI Chat-agentvelgeren.
...
For å bruke OpenCtrlC som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), legg til følgende i Neovim-konfigurasjonen:
...
Denne konfigurasjonen setter opp CodeCompanion.nvim til å bruke OpenCtrlC som ACP-agent for chat.
...
OpenCtrlC fungerer på samme måte via ACP som i terminalen. Alle funksjoner støttes:
...
- MCP servere konfigurert i OpenCtrlC-konfigurasjonen

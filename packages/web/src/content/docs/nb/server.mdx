---
title: Tjener
description: Samhandle med OpenCtrlC-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `openctrlc serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en OpenCtrlC-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `openctrlc.local` |
...
Sett `OPENCTRLC_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `openctrlc` som standard, eller sett `OPENCTRLC_SERVER_USERNAME` for å overstyre det. Dette gjelder både `openctrlc serve` og `openctrlc web`.
...
Når du kjører `openctrlc` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk OpenCtrlC-serveren til å samhandle med OpenCtrlC programmatisk.
:::

Denne arkitekturen lar OpenCtrlC støtte flere klienter og lar deg samhandle med OpenCtrlC programmatisk.

Du kan kjøre `openctrlc serve` for å starte en frittstående server. Hvis du har
OpenCtrlC TUI kjører, vil `openctrlc serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av OpenCtrlC [IDE](/docs/ide) plugins.
...
openctrlc-serveren viser følgende APIer.

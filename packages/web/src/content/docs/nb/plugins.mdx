...
description: Skriv dine egne programtillegg for å utvide OpenCtrlC.
---

Plugins lar deg utvide OpenCtrlC ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til OpenCtrlC.
...
- `.openctrlc/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/openctrlc/node_modules/`.
...
1. Global konfigurasjon (`~/.config/openctrlc/openctrlc.json`)
2. Prosjektkonfigurasjon (`openctrlc.json`)
3. Global plugin-katalog (`~/.config/openctrlc/plugins/`)
4. Prosjektpluginkatalog (`.openctrlc/plugins/`)
...
OpenCtrlC kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En OpenCtrlC SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide OpenCtrlC.
...
:::note
Hvis du bruker OpenCtrlC-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre OpenCtrlC fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til OpenCtrlC:
...
`tool`-hjelperen lager et tilpasset verktøy som OpenCtrlC kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for OpenCtrlC sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://openctrlc.pages.dev/docs/sdk) for detaljer.

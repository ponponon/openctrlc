...
OpenCode Desktop kjører en lokal OpenCode-server (`openctrlc-cli` sidevognen) i bakgrunnen. De fleste problemene er forårsaket av en plugin som fungerer dårlig, en ødelagt cache eller en dårlig serverinnstilling.
...
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\opencode\openctrlc.jsonc`

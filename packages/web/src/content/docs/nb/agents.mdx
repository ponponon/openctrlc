...
Det er to typer agenter i OpenCtrlC; primære agenter og underagenter.
...
OpenCtrlC kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
OpenCtrlC kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

OpenCtrlC kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `openctrlc.json`:
...
Hvis ingen temperatur er spesifisert, bruker OpenCtrlC modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale OpenCtrlC-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i OpenCtrlC-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [den konfigurerte leverandøren](/docs/providers), vil du bruke `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `openctrlc models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/ponponon/openctrlc).
:::

...
Du kan legge til eksterne verktøy i OpenCtrlC ved å bruke _Model Context Protocol_ eller MCP. OpenCtrlC støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [OpenCtrlC Config](https://openctrlc.pages.dev/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="openctrlc.jsonc" {6}
{
...
```jsonc title="openctrlc.jsonc" {15}
{
  "$schema": "https://openctrlc.pages.dev/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
OpenCtrlC håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil OpenCtrlC:
...
Hvis serveren krever autentisering, vil OpenCtrlC be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `openctrlc mcp auth <server-name>`.
...
```json title="openctrlc.json" {7-11}
{
  "$schema": "https://openctrlc.pages.dev/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
openctrlc mcp auth my-oauth-server
```
...
```bash
openctrlc mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil OpenCtrlC lagre tokens sikkert i `~/.local/share/openctrlc/mcp-auth.json`.
...
openctrlc mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i OpenCtrlC, sammen med innebygde verktøy. Så du kan administrere dem gjennom OpenCtrlC-konfigurasjonen som alle andre verktøy.
...
```json title="openctrlc.json" {11, 14-18}
{
  "$schema": "https://openctrlc.pages.dev/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
openctrlc mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble OpenCtrlC til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver OpenCtrlC-samtale oppført som en separat økt.

...
Place your global OpenCode config in `~/.config/openctrlc/openctrlc.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.
...
On macOS, OpenCode reads managed preferences from the `ai.opencode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically. This is the existing external/desktop contract; Task 3 does not migrate it.

...
2. **Global config** (`~/.config/opencode/openctrlc.json`) - user preferences
...
:::note
The `.openctrlc` and `~/.config/opencode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Place your global OpenCode config in `~/.config/opencode/openctrlc.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.
...
On macOS, OpenCode reads managed preferences from the `ai.openctrlc.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.
...
Use the `ai.openctrlc.managed` PayloadType. The OpenCode config keys go directly in the payload dict:
...
You can also define agents using markdown files in `~/.config/opencode/agents/` or `.openctrlc/agents/`. [Learn more here](/docs/agents).
...
You can also define commands using markdown files in `~/.config/opencode/commands/` or `.openctrlc/commands/`. [Learn more here](/docs/commands).
...
Place plugin files in `.openctrlc/plugins/` or `~/.config/opencode/plugins/`. You can also load plugins from npm through the `plugin` option.

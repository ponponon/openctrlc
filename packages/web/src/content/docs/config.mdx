---
title: Config
description: Using the OpenCtrlC JSON config.
---

You can configure OpenCtrlC using a JSON config file.

---

## Format

OpenCtrlC supports both **JSON** and **JSONC** (JSON with Comments) formats.
...
2. **Global config** (`~/.config/openctrlc/openctrlc.json`) - user preferences
3. **Custom config** (`OPENCTRLC_CONFIG` env var) - custom overrides
4. **Project config** (`openctrlc.json` in project) - project-specific settings
5. **`.openctrlc` directories** - agents, commands, plugins
6. **Inline config** (`OPENCTRLC_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/openctrlc/` on macOS) - admin-controlled
...
:::note
The `.openctrlc` and `~/.config/openctrlc` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Place your global OpenCtrlC config in `~/.config/openctrlc/openctrlc.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/openctrlc/tui.json`.
...
Add `openctrlc.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When OpenCtrlC starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `OPENCTRLC_CONFIG` environment variable.
...
Specify a custom config directory using the `OPENCTRLC_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.openctrlc` directory, and should
...
The custom directory is loaded after the global config and `.openctrlc` directories, so it **can override** their settings.
...
Drop an `openctrlc.json` or `openctrlc.jsonc` file in the system managed config directory:

| Platform | Path                                      |
| -------- | ----------------------------------------- |
| macOS    | `/Library/Application Support/openctrlc/` |
| Linux    | `/etc/openctrlc/`                         |
| Windows  | `%ProgramData%\openctrlc`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, OpenCtrlC reads managed preferences from the `ai.openctrlc.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically. This is the existing external/desktop contract; Task 3 does not migrate it.

OpenCtrlC checks these paths:

1. `/Library/Managed Preferences/<user>/ai.openctrlc.managed.plist`
2. `/Library/Managed Preferences/ai.openctrlc.managed.plist`

The plist keys map directly to `openctrlc.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.openctrlc.managed` PayloadType. The OpenCtrlC config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.openctrlc.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.openctrlc.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.openctrlc</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`openctrlc.pages.dev/config.json`**](https://openctrlc.pages.dev/config.json).

TUI config uses [**`openctrlc.pages.dev/tui.json`**](https://openctrlc.pages.dev/tui.json).
...
```json title="tui.json"
{
  "$schema": "https://openctrlc.pages.dev/tui.json",
  "scroll_speed": 3,
  "scroll_acceleration": {
    "enabled": true
  },
...
Use `OPENCTRLC_TUI_CONFIG` to point to a custom TUI config file.
...
Legacy `theme`, `keybinds`, and `tui` keys in `openctrlc.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `openctrlc serve` and `openctrlc web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your OpenCtrlC server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `openctrlc.local`. Useful for running multiple instances on the same network.
...
If not specified, OpenCtrlC will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your OpenCtrlC config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, OpenCtrlC tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny OpenCtrlC actions on configured resources. Currently, policies can control which providers OpenCtrlC may use.
...
OpenCtrlC normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, OpenCtrlC omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/openctrlc/agents/` or `.openctrlc/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, OpenCtrlC will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`openctrlc run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/openctrlc/commands/` or `.openctrlc/commands/`. [Learn more here](/docs/commands).
...
OpenCtrlC uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
OpenCtrlC will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, openctrlc **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend OpenCtrlC with custom tools, hooks, and integrations.

Place plugin files in `.openctrlc/plugins/` or `~/.config/openctrlc/plugins/`. You can also load plugins from npm through the `plugin` option.
...
This is useful when you want to restrict OpenCtrlC to only use specific providers rather than disabling them one by one.

...
import { FileSystemWatcher } from "@openctrlc/schema/filesystem-watcher"
...
declare const OPENCTRLC_LIBC: string | undefined
...
const watcher = lazy((): typeof import("@parcel/watcher") | undefined => {
  try {
    const libc = typeof OPENCTRLC_LIBC === "undefined" ? undefined : OPENCTRLC_LIBC
...
const layer = Layer.effect(
  Service,
  Effect.gen(function* () {
    if (yield* Flag.OPENCTRLC_EXPERIMENTAL_DISABLE_FILEWATCHER) return Service.of({})
    ...
    if (location.vcs && (yield* Flag.OPENCTRLC_EXPERIMENTAL_FILEWATCHER)) {

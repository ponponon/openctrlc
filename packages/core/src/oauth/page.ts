...
// etc.).
//
// The visual language mirrors the OpenCtrlC app: the design tokens are a curated
...
(function(){
  var card=document.getElementById("oc-card"),headline=document.getElementById("oc-headline"),message=document.getElementById("oc-message"),detail=document.getElementById("oc-detail"),footnote=document.getElementById("oc-footnote");
   function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?(${scriptString(`${Brand.name} couldn't finish connecting to `)}+PROVIDER+"."):${scriptString(`${Brand.name} couldn't complete authorization.`)};if(text){detail.textContent=text;detail.hidden=false}footnote.textContent=${scriptString(`Close this window and try again from ${Brand.name}.`)} }
   function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?(${scriptString(`${Brand.name} is now connected to `)}+PROVIDER+"."):${scriptString(`${Brand.name} is now authorized.`)};detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
// OpenCtrlC wordmark — same path geometry as packages/ui/src/components/logo.tsx (Logo).
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 234 42" fill="none" aria-label="${Brand.name}" role="img">

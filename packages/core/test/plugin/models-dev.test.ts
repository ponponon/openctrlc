...
import { ModelsDevPlugin } from "@openctrlc/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.OPENCTRLC_MODELS_PATH,
          disabled: Flag.OPENCTRLC_DISABLE_MODELS_FETCH,
        }
        Flag.OPENCTRLC_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.OPENCTRLC_MODELS_PATH = previous.path
          Flag.OPENCTRLC_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})

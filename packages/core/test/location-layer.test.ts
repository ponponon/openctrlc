...
import { define } from "@openctrlc/plugin/v2/effect"
...
import { LocationServiceMap } from "@openctrlc/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "openctrlc.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "openctrlc.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",

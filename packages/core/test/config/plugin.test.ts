...
import { ConfigExternalPlugin } from "@openctrlc/core/config/plugin/external"
...
import { PluginHost } from "@openctrlc/core/plugin/host"
...
describe("ConfigExternalPlugin", () => {
  it.live("resolves and loads a configured Promise plugin with options", () =>
    Effect.gen(function* () {
      ...
      const document = path.join(import.meta.dir, "openctrlc.json")
          ...
          Config.Service.of({
            entries: () =>
              Effect.succeed([
                new Config.Document({
                  type: "document",
                  path: path.join(import.meta.dir, "openctrlc.json"),
                  info: decode({
                    plugins: [
                      {

...
// test/preload.ts pins OPENCTRLC_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.OPENCTRLC_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.OPENCTRLC_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.OPENCTRLC_MODELS_PATH = undefined
  Flag.OPENCTRLC_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.OPENCTRLC_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.OPENCTRLC_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
  ...
  it.live("get() recovers from a corrupted cache file by fetching a fresh catalog", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          Flag.OPENCTRLC_DISABLE_MODELS_FETCH = false
        }),

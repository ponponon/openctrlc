...
const getBase = (appId: string): Configuration => ({
  artifactName: "openctrlc-${os}-${arch}.${ext}",
  directories: {
    output: "dist",
  ...
  files: ["out/**/*", "resources/**/*", "!resources/openctrlc*"],
  extraResources: [
    ...(channel === "dev"
      ? [
          {
  ...
  mac: {
    ...
    signIgnore: ["\\.pak$", "(^|[-/])(linux|win32|freebsd|android|aix|sunos)([-/]|$)"],
    notarize: true,
    target: ["dmg", "zip"],
  },
  dmg: {
  ...
  switch (channel) {
    case "dev": {
      return {
        ...
        rpm: { packageName: "openctrlc-dev", fpm: [metainfoFpm(appId)] },
      }
    }
    case "beta": {
      return {

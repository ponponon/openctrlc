...
const releaseId = process.env.OPENCTRLC_RELEASE
if (!releaseId) throw new Error("OPENCTRLC_RELEASE is required")

const version = process.env.OPENCTRLC_VERSION
if (!version) throw new Error("OPENCTRLC_VERSION is required")
...
function link(raw: string) {
  ...
  const tag = process.env.OPENCTRLC_CHANNEL === "beta" ? "beta" : `v${version}`
...
const macxTarGz = "openctrlc-mac-x64.app.tar.gz"
const macaTarGz = "openctrlc-mac-arm64.app.tar.gz"

...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier OpenCtrlC launch", () => {
    expect(hasExistingAppState([file("openctrlc.settings")])).toBe(true)
    expect(hasExistingAppState([file("openctrlc.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("openctrlc")])).toBe(true)
  })
})

...
const TEST_ONBOARDING = process.env.OPENCTRLC_TEST_ONBOARDING === "1"
const SIDECAR_VERSION = process.env.OPENCTRLC_SIDECAR_V2 === "1" ? "v2" : "v1"
...
const main = Effect.gen(function* () {
  ...
  const onboardingTestRoot = ((): string | undefined => {
    ...
    const root = join(tmpdir(), `${Brand.cli}-onboarding-${randomUUID()}`)
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : Brand.name)
  ...
  const loadingTask = yield* Effect.gen(function* () {
    ...
    const port = yield* Effect.gen(function* () {
      const fromEnv = process.env.OPENCTRLC_PORT
      if (fromEnv) {

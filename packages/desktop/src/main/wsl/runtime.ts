...
export async function installWslOpenctrlc(version: string, distro: string, opts?: RunWslOptions) {
  return runInteractiveCommand(
    resolveSystem32Command("wsl.exe"),
    wslArgs(
      ["bash", "-lc", `curl -fsSL https://raw.githubusercontent.com/ponponon/openctrlc/dev/install | bash -s -- --version ${shellEscape(version)}`],
      distro,
    ),
    withTimeout(opts, DEFAULT_WSL_INSTALL_TIMEOUT_MS),
    DEFAULT_WSL_INSTALL_TIMEOUT_MS,
...
export async function resolveWslOpenctrlc(distro: string, opts?: RunWslOptions) {
  return firstLine(
    (
      await runWslSh(
        'if [ -x "$HOME/.openctrlc/bin/openctrlc" ]; then printf "%s\\n" "$HOME/.openctrlc/bin/openctrlc"; fi',
        distro,
        opts,
      )
    ).stdout,

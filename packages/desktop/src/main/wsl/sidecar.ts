...
import { type WslCommandLine, resolveWslOpenctrlc, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const openctrlc = await resolveWslOpenctrlc(distro)
  if (!openctrlc) throw new Error(nativeT("desktop.wsl.error.openctrlcNotInstalled", { distro }))

  const port = await allocatePort()
  const password = randomUUID()
  const username = Brand.cli
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export OPENCTRLC_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export OPENCTRLC_CLIENT=desktop",
    `export OPENCTRLC_SERVER_USERNAME=${shellEscape(username)}`,
    `export OPENCTRLC_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(openctrlc)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")

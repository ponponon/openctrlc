...
import { expectOpenctrlcVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveOpenctrlc?: typeof resolveWslOpenctrlc
...
export function createWslServersController(
  ...
  const setOpenctrlcCheck = (distro: string, check: WslOpenctrlcCheck) => {
    setState({
      openctrlcChecks: {
        ...state.openctrlcChecks,
        [distro]: check,
      },
    })
  }

  const checkOpenctrlc = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveOpenctrlc ?? resolveWslOpenctrlc)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return openctrlcCheck(distro, resolved, version, appVersion)
  }

  const refreshOpenctrlcCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setOpenctrlcCheck(distro, await checkOpenctrlc(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const openctrlcChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.openctrlcChecks[distro])
        .map(async (distro) => [distro, await checkOpenctrlc(distro, opts)] as const),
    )
    if (openctrlcChecks.length) {
      setState({ openctrlcChecks: { ...state.openctrlcChecks, ...Object.fromEntries(openctrlcChecks) } })
    }
  }
  ...
  const refreshOpenctrlcCheckBackground = (id: string, distro: string) => {
    void checkOpenctrlc(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setOpenctrlcCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl openctrlc check failed", { id, distro, message })
      })
  }

  const refreshOpenctrlcChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkOpenctrlc(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setOpenctrlcCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl openctrlc check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshOpenctrlcCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installOpenctrlc(name: string) {
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.openctrlcChecks, distro) : {}),
      })
    },

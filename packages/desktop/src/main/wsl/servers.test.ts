...
let releaseOpenctrlcResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectOpenctrlcVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectOpenctrlcVersion("1.14.35", "1.16.2")).toThrow(
    "OpenCtrlC update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})
...
test("clears cached distro probes when removing a WSL server", () => {
  ...
  ).toEqual({ distroProbes: {}, openctrlcChecks: {} })
})
...
test("ignores stale background OpenCtrlC checks after removing a WSL server", async () => {
  ...
  await waitFor(() => !!releaseOpenctrlcResolve)
  await controller.removeServer("wsl:Debian")
  releaseOpenctrlcResolve?.()
...
test("probes addable distros in parallel before checking OpenCtrlC", async () => {
  ...
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveOpenctrlc: async (distro) => {
      openctrlc.push(distro)
      return "/home/me/.openctrlc/bin/openctrlc"
    },
  })
...
test("does not check OpenCtrlC in addable distros that cannot execute commands", async () => {
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(openctrlc).toEqual(["Debian"])
  expect(Object.keys(controller.getState().openctrlcChecks)).toEqual(["Debian"])
})
...
function testControllerOptions() {
  return {
    ...
    resolveOpenctrlc: async () => {
      await new Promise<void>((resolve) => {
        releaseOpenctrlcResolve = resolve
      })
      return "/home/me/.openctrlc/bin/openctrlc"
    },
  }

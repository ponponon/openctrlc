...
export function registerWslIpcHandlers(controller: WslServersController) {
  ...
  ipcMain.handle("wsl-servers-install-openctrlc", (_event: IpcMainInvokeEvent, name: string) =>
    controller.installOpenctrlc(requireWslIpcString("distro", name)),
  )
...
function registerUnavailableWslIpcHandlers() {
  ...
  ipcMain.handle("wsl-servers-install-openctrlc", unavailable)

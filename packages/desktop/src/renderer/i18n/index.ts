...
const localized: Partial<Record<Locale, typeof desktopEn>> = {
  zh: desktopZh,
  ja: desktopJa,
  ko: desktopKo,
}
...
function build(locale: Locale): Dictionary {
  const dictionary = localized[locale]
  if (!dictionary) return base
  return { ...base, ...i18n.flatten(dictionary) }
}
...
export function initI18n(): Promise<Locale> {
  ...
  const promise = (async () => {
    const raw = await window.api.storeGet("openctrlc.global.dat", "language").catch(() => null)

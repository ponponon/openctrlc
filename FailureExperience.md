...
以后判断“是否已经持久化”时，必须基于完整会话历史检查是否存在快照，不能依赖已经为模型裁剪、重排或压缩过的上下文列表；回归测试应覆盖多轮用户消息以及快照滚出最近分页窗口的场景，并在涉及 compaction 时验证压缩后续聊不会重复写入。
...
为了让上下文面板显示有效 System Prompt，曾在每个模型回合完成请求准备后，把包含项目指令、Skill、MCP
和环境信息的完整提示词写入当前用户消息。这个展示需求实际上只需要会话级的一份快照，却把诊断数据错误地

...
1. If `.openctrlc/references/effect-smol` is missing, clone `https://github.com/Effect-TS/effect-smol` there. Do this in the project, not in the skill folder.
2. Search `.openctrlc/references/effect-smol` for exact APIs, examples, tests, and naming patterns before answering or implementing Effect-specific code.
...
- Do not answer from memory. Verify against `.openctrlc/references/effect-smol` or nearby code first.
...
- Run tests from package directories such as `packages/openctrlc`; never run package tests from the repo root.
